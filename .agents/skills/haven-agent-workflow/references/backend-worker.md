...
Start by reading:
- `AGENTS.md`
- any client-specific repository instructions that are present, such as `CLAUDE.md`
- any feature-specific docs or plans named by the captain

Collaboration rules:
- You are not alone in the codebase.
- Edit only the files explicitly assigned by the captain.
...
- Anything over the remaining budget is declined before any money moves — never held for a human to approve later. There is no approval queue on any live rail, so do not build one, and do not add a routing branch for one.
