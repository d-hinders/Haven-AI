...
4. **Return the verdict line the Baseline change gate reads**, exactly this shape, in your verdict so the author can paste it into the PR body:
   ...
   Name only the baselines you opened and passed. For a blocked one, write `design-review verdict: changes requested @ <head-sha> -- baselines: <name>`. The newest verdict decides per baseline, and a named line outranks `*` for that name: your block vetoes any earlier `passed` at or before your sha (a `*` one included), and only a later `passed` from a re-review that names the file clears it (a `*` pass does not) — nobody deletes lines (#3301). Keep free text off these lines: the verdict word is the whole text before `@`, so `passed` and `approved` verify and `passed-with-nits` does not. The sha is the head you reviewed: a later commit that touches that PNG, or a rebase, voids the line, so a re-commit needs a re-review.
