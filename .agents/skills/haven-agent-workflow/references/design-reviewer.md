...
**The root you were handed (#2455, #2601).** This file carries no isolation section of its own, because the captain hands you a root and the rule has one home: run `scripts/ci/review-isolation.mjs` before you look at anything, quote its `root`/`head`/`base` in your verdict, and `git worktree remove <review-root> --force` after it — kept on a `blocked` verdict or a guard refusal. All of it, with the reasons, is in [`reviewer.md`](reviewer.md) § *Before anything else* and § *After the verdict*; do not restate it here.
