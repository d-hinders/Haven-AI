...
- After the verdict, `git worktree remove <review-root> --force` — the pass that made the root is the only actor that knows it is over, and 173 worktrees accumulated before that was written down (#2601). It STAYS on a `blocked` verdict or a guard refusal; the rule and both exceptions are in [`reviewer.md`](reviewer.md) § *After the verdict*.
