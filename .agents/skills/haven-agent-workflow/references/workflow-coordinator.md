...
- Decide which work should stay with the captain and which work can be delegated, then give the captain a directly actionable plan.
- Prefer one captain session per feature branch.
- Use the client's available task or plan tracking capability when it helps track the agreed plan, ownership boundaries, and verification steps.
...
Recommend roles only when they materially help:
