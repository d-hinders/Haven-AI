...
3. Run `npx -y <connector_package> --doctor --runtime hermes` again, with the same package step 1 used: `--doctor` checks the installed signer against the manifest of the connector running it, so a doctor from another channel reports a false `signer_runtime` mismatch, and the `--repair` it then suggests would reinstall that channel's signer. A clean reconnection report exits zero with no `!` line other than the `signer_runtime` and `signer_runtime_unused` (#3123, unreferenced runtime directories — prune later) advisories, which both exit zero: since #3121 that cross-channel mismatch is an advisory (`!`, "intact, but outdated") which also exits zero, so read the `signer_runtime` line rather than the exit code alone; `identity_match` passes, and no `superseded` agent is named. Retained tombstone directories may still be reported as `retired` until every long-lived host has restarted.
