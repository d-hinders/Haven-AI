...
- **Signer Readiness Gates.** Wallet, passkey, `useActiveSigner`, `useSafeOperationGate`, `OnchainActionGate`, `WalletButton`, and wallet-approval copy changes do not treat `address` or `isConnected` alone as signer readiness. EOA readiness matches the signer hook's `address && walletClient` requirement. Tests cover address-present / walletClient-missing with a visible recovery action.
...
- **Inline Gate Placement.** A separate `<OnchainActionNotice />` renders above the action row, not inside `flex-1`, with `showNotice={false}` on `OnchainActionGate`; `EditAgentModal` is the canonical hoisted pattern. `NetworkGate` intentionally keeps its mismatch hint and replacement network action in place.
