...
2. Check for existing work: `gh issue develop <issue-nr> --list` (GitHub's linked branches, #3180 — rely on it for the window from another session's claim until its PR exists; on #3180 the link was gone once the PR was opened, so an empty list is never "no overlap", and who owns a listed branch is read from the claim comment), then — still mandatory — `gh pr list --search "<issue-nr>"` and `git ls-remote --heads origin | grep <issue-nr>`, the only reads that see a branch nobody linked.
...
**Claim before you build:** comment `🔒 CLAIM #<issue> — branch <name> — touches: <files/areas> — <session owner>` on the issue itself; once the claim stands, create your branch **linked to the issue** — `gh issue develop <issue> --name <branch> --base dev` (run against a branch that already exists on origin it links instead of creating) — so the sidebar and `gh issue develop --list` carry it until your PR exists (#3180; on #3180 the link was gone once the PR was opened). The comment stays the record (it carries `touches:` and the owner; a claim without a linked branch is still a claim, a linked branch without a claim is not). If your claim is refused or you abandon the work, delete the branch you linked (`git push origin --delete <branch>`) with your `🔓 RELEASE`; ALSO post it to #1289 when the work touches shared surfaces (`packages/mcp-server/src/tools*` — the facade and everything under `tools/`, since #2807–#2809 split the hosted surface across several files — demo-merchant-mcp, migrations, release trains, `db-mock-baseline.json`, contract docs).
