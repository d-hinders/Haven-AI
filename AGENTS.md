...
**A pull request that closes someone else's issue does not merge** (#3179): the `PR ownership gate` check reads every issue your PR would close and fails while one of them is open and held by another session — assigned to them, or under their live claim by the #3178 rule — naming the holder and linking the claim. Two ways out: a handover in #1289 (the holder posts `🔓 RELEASE`), or `Refs #<issue>` instead of `Closes`. Drafts are not gated. (Whether the check is REQUIRED on `dev` is a ruleset setting the owner applies — see the ruleset inventory in `docs/contributing/autonomous-pr-loop.md`.)
