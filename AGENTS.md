...
2. Check for existing work: `gh issue develop <issue-nr> --list` (GitHub's linked branches — a branch you did not create is another session's, #3180), then `gh pr list --search "<issue-nr>"` and `git ls-remote --heads origin | grep <issue-nr>` for branches nobody linked.
...
**Claim before you build:** create your branch **linked to the issue** — `gh issue develop <issue> --name <branch> --base dev` (the same command run against a branch that already exists on origin links it instead of creating one) — so the issue sidebar and `gh issue develop --list` carry it natively (#3180); then comment `🔒 CLAIM #<issue> — branch <name> — touches: <files/areas> — <session owner>` on the issue itself. The comment stays the record (it carries `touches:` and the owner; a claim without a linked branch is still a claim, a linked branch without a claim is not); ALSO post it to #1289 when the work touches shared surfaces (`packages/mcp-server/src/tools*` — the facade and everything under `tools/`, since #2807–#2809 split the hosted surface across several files — demo-merchant-mcp, migrations, release trains, `db-mock-baseline.json`, contract docs).
