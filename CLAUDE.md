...
- **Safe AllowanceModule rail — RETIRED (#1440).** The on-chain policy primitive for Safe accounts (dev-pilot). Retired in the #834 shape: machinery deleted, fail-closed 410 kept, typed rail seam kept for reversibility. Three closures, in order:
  ...
  **What deliberately SURVIVES, and why** — this is the part a sweep gets wrong in the direction of overclaiming:
  - **Accounts and balances stay READABLE — with THREE later narrowings, the last of which effectively ends it.** The original promise: `GET /user/safes`, rename, re-default, unlink and balances untouched, a legacy account rendering in full with an inert `RetiredRailNotice` where the spend action used to be. #2020 reversed the `GET /machine-payments/allowances` half (410 on the retired rail, owner call on #2020); #2055 removed the approval queue's history from transaction/x402/activity reads (owner call on #2021) — evidence rows preserved, but approval-executed legacy payments no longer appear in history responses. **#2413 then stopped Haven rendering legacy accounts at all** (owner decision 2026-09-02: retirement is deletion, not accommodation). Six list queries — the three account lists, the agent list, and the two dashboard lists — filter to `account_type = 'delegator_hybrid'`, so `GET /user/safes` returns no legacy account, the agent list returns none of their agents, and `RetiredRailNotice` is DELETED along with every `account_type` branch behind it. Read the readability promise as history: the rows are untouched and a direct database query still finds them, but no Haven surface displays them and nothing on-chain changed. Dropping the rows outright remains a separate, still-open decision, blocked on `payment_intents`' RESTRICT foreign key.
...
Five packages are published to npm: `@haven_ai/sdk`, `@haven_ai/signer`, `@haven_ai/mcp`, `@haven_ai/connect` (the connector the dashboard hands out — `npx @haven_ai/connect@alpha` in production; since #2422 a non-production deployment can hand out another channel via `HAVEN_CONNECTOR_CHANNEL`, and the setup response carries the spec it used in `connector_package`), and `@haven_ai/cli`. `mcp-server`, `backend`, and `frontend` are NOT on npm — they deploy from branches; `@haven_ai/core` is a **private workspace package** (never published — the shared kernel backend/frontend consume with a `"*"` pin, outside `release-bump.mjs` entirely) (Railway/Vercel): `main` → production, and the `dev` integration branch → the shared **dev environment** (see [`docs/operations/dev-environment.md`](docs/operations/dev-environment.md)).
...
- **Never run `npm publish` by hand.** To cut a release, run `npm run release:bump -- <version>` (e.g. `0.1.17-alpha.0`), commit on a release branch, and open the PR **into `dev`, never `main`** — `dev-gate` fails a `release/*` branch aimed at `main` by design. Merging to `dev` does not publish **the release**: the **Publish packages** workflow (`.github/workflows/publish.yml`) publishes it on the subsequent **`dev → main` promotion**, choosing the dist-tag from the version (prerelease → `alpha`, stable → `latest`) and skipping any version already on npm. Since [#2421](https://github.com/d-hinders/Haven-AI/issues/2421) (epic #2420) the same workflow ALSO fires on a package-touching push to `dev` and publishes a `0.0.0-dev.<ts>.<sha>` snapshot of all five packages under a separate **`dev`** dist-tag, built in a throwaway CI tree with nothing committed. The two channels cannot cross — a snapshot can reach neither `alpha` nor `latest`, and the `main` path refuses a `0.0.0-dev.*` version — and the enforcement points are named in that workflow's header comment. One file, because npm trusted publishing is pinned to the workflow filename.
- **Where a `@dev` snapshot comes from, and how to use one:** [`docs/operations/package-dev-channel.md`](docs/operations/package-dev-channel.md) — the merge → `publish.yml` → `npm view` → install → `--doctor` loop for testing a package change on the dev environment without a release, the #2424 local runtime-spec override for a build that has not merged, and the owner-only checklist (npm trusted publishers, then the first package-touching push, then `HAVEN_CONNECTOR_CHANNEL=dev` on the dev services — in that order). It changes nothing above: the promotion still publishes the release, and nobody runs `npm publish` by hand.
...
**Design-quality workflow v2 (epic #904):** design-lint is one of five guards. Also enforced: **visual regression** — every `e2e/**/*.visual.spec.ts` is pixel-compared against committed Linux baselines (update baselines via the *Update visual baselines* workflow_dispatch on the PR branch, never commit macOS-rendered ones). That is `/design-system` **scoped only** (top bar + sidebar clips; the whole-page `/design-system` capture was removed by #2635 — it produced 5 of the last 12 sampled *Design visual regression* CI failures, all a 15s pixel-comparison timeout on a 22.7M-pixel page, none a real regression), element clips on `/agents`, and — since #2318 — whole-page captures of `/dashboard` and `/transactions`. **Where the gate is required:** today it is a required check on both `dev` and `main`; epic #2632's owner step O2 — a GitHub-settings action, independent of any pull request — moves it to required-on-`main`-only, advisory-on-`dev`. Once O2 lands, a red tick on a `dev` PR is informational, not a merge blocker; the same red on a `main`-bound PR still blocks. **Read the green tick as narrowly as it is true:** the job prints the baselines it actually compared into its own summary (`scripts/ci/visual-baseline-inventory.mjs`), and every screen absent from that list has no pixel baseline at all; **design-system coupling** (#898) — a new `ui/`/`haven/` export must appear on `/design-system` in the same PR (blocking on every PR via the *Design-system coupling (strict)* check, #1023, with a sticky comment explaining the finding; escape: `// design-system-exempt: <reason>`); **copy-lint** (#902) — blocking, shrink-only baseline in `packages/frontend/copy-lint-baseline.json` (`npm run lint:copy`); **rendered review** (#900) — `area:frontend` diffs get a `haven-design-reviewer` pass over the `npm run screenshot` evidence in addition to `haven-reviewer`, and a finding from either pauses auto-merge; and the **pattern-absorption preflight** (#901) — extract a repeated markup shape into a primitive on its 2nd occurrence. A weekly non-gating `qa-explore-ui` cadence (#903, `docs/operations/qa-explore-ui-cadence.md`) feeds UX findings into the backlog — and since #2538 it carries a **second** scenario, `agent-onboarding-cold`, which points the other way: a fresh agent given only a dev URL, scored on whether it can get its user set up. Both are owner-armed Routines, never CI jobs; the cold one additionally cannot be a CI job, because an agent inside this repository's own runner is not cold and its discovery score would stop meaning what it says. Full process: `docs/contributing/ship-playbooks/frontend.md`.
