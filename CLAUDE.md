...
- **Safe AllowanceModule rail — RETIRED (#1440).** The on-chain policy primitive for Safe accounts (dev-pilot). Retired in the #834 shape: machinery deleted, fail-closed 410 kept, typed rail seam kept for reversibility. Three closures, in order:
  ...
  **What deliberately SURVIVES, and why** — this is the part a sweep gets wrong in the direction of overclaiming:
  - **Accounts, balances and transaction history stay READABLE.** `GET /user/safes`, rename, re-default, unlink, balances and history are untouched, as is `GET /machine-payments/allowances`, which reports state rather than exercising authority. A legacy account renders in full, with an inert `RetiredRailNotice` where the spend action used to be.
  - **`rails/allowance-module.ts` survives, trimmed to reads-only.** `getProvider` / `getRelayerWallet` / `getTokenBalance` are not AllowanceModule code at all — they are shared with `rails/sweep.ts`, the delegate-balance monitor and the #946 EIP-3009 bridge. `lib/safe-tx.ts` survives the same way on the frontend.
...
4. **On-chain refusal, not an approval queue** — a payment outside the budget, recipient pin or expiry **reverts during gas estimation**. The legacy rail's human-circuit-breaker approval queue is retired with it (#1440): approvals are readable and rejectable, never approvable, and nothing writes a new one. A human circuit breaker for high-value actions on the delegation rail is **future work**, not a shipped layer
