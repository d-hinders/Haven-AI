...
1. **Enforced by GitHub, for everyone, whatever opened the PR.** Tests, typecheck and build; `Docs front-matter & agent skills`; `Design-system coupling (strict)`; `Design visual regression`; `Banned product-copy terms`; `Contract-doc coupling`; `dev-gate` (+ `qa-freshness` on promotion); `CODEOWNERS` on `db/migrations/`. Not optional, not tool-dependent, not skippable by working differently. The current list lives in the ruleset inventory in [`docs/contributing/autonomous-pr-loop.md`](docs/contributing/autonomous-pr-loop.md) — that file, not this one.
