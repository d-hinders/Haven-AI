...
prompt, and the only onboarding path there is. The **signer set** is user-managed
...
- **`allowances` on an agent is a derived VIEW, not the policy** — projected
  ...
  enforcement is the delegation itself. `allowance_amount` is **human-decimal** here,
...
- **Enforcement is on-chain, and over-budget REVERTS — it does not queue.**
  ...
  estimation. No approval queue, no `requires_approval_above` knob, no per-agent
  monthly or per-tx limit.
...
- **Lifecycle.** Connect-modal agents start `pending_approval` and flip to
  `active` inside the first budget-grant activation; `POST /agents` starts
  `active`.
...
`retired_allowance` — and only the first executes anything. Both retirements
...
requires an **open (unpinned) budget** — pinned agents are erc7710-only.
...
- **Chain:** **Base (chain ID 8453) is the primary / default network**; Gnosis
  ...
  default; a guard test flags new bare numeric chain fallbacks.
...
- **A prerelease also gets `latest` pointed at it**, because a bare
  ...
  that job rather than cutting a version.
...
limit. The captain owns product judgment, shared files, git hygiene and
integration, plus the gravity files [`AGENTS.md`](AGENTS.md) enumerates.
...
the next epic rather than shipping one.
...
2. **What `ship-next` adds on top.** Playbook routing by `area:*` /
   ...
   acceptance-criteria evidence. Judgement work: **CI does none of it**, and no
   check will say it was skipped.
