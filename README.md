...
- **Two channels, one file ([#2421](https://github.com/d-hinders/Haven-AI/issues/2421), epic #2420):** the paragraph above is the **prod** channel (`main` → `alpha`/`latest`). The same workflow also fires on a package-touching push to **`dev`** and publishes a `0.0.0-dev.<YYYYMMDDHHMM>.<shortsha>` snapshot of all five packages under the **`dev`** dist-tag — built by the ordinary bump script in a throwaway CI tree, with nothing committed. `0.0.0-` sorts below every real version, so no `^0.1.x` range resolves to a snapshot by accident. A snapshot can reach neither `alpha` nor `latest`, and the `main` path refuses a `0.0.0-dev.*` version; the enforcement points are named in the workflow's header comment. One file rather than two because npm's trusted-publisher entries are pinned to the workflow **filename**.
