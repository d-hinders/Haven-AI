...
> **Not to be confused with [`AGENTS.md`](AGENTS.md).** That file is for
> contributors to *this repository* — build, test and review conventions for
> changing Haven itself. It is not a guide for an agent that wants to USE
> Haven to pay for something; this section is.
...
sequence is recorded once in the
[decision log](docs/archive/decision-log.md#2026-08-14--retire-the-safe-rail-entirely-1440)
and is not restated here (#2640).
Existing Safe rows are untouched and still readable to a direct database query, but
since #2413 no account, agent or dashboard surface displays them — balances, tokens and agents no
longer render there. Transaction history is the named exception: `GET /transactions` carries no rail
predicate on either its account or its agent list, so legacy names still appear in its picklists. Any residual AllowanceModule permission remains on-chain and outside Haven until
...
- **Two channels, one file ([#2421](https://github.com/d-hinders/Haven-AI/issues/2421), epic #2420):** the paragraph above is the **prod** channel (`main` → `alpha`/`latest`). The same workflow also fires on a package-touching push to **`dev`** and publishes a `0.0.0-dev.<YYYYMMDDHHMM>.<shortsha>` snapshot of all five packages under the **`dev`** dist-tag — built by the ordinary bump script in a throwaway CI tree, with nothing committed. `0.0.0-` sorts below every real version, so no `^0.1.x` range resolves to a snapshot by accident. A snapshot can reach neither `alpha` nor `latest`, and the `main` path refuses a `0.0.0-dev.*` version; the enforcement points are named in the workflow's header comment. Loop and owner steps: [`docs/operations/package-dev-channel.md`](docs/operations/package-dev-channel.md). One file rather than two because npm's trusted-publisher entries are pinned to the workflow **filename**.
- **Auth for PUBLISHING:** [npm Trusted Publishing (OIDC)](https://docs.npmjs.com/trusted-publishers) — **no long-lived credential**. The workflow grants the publish job `id-token: write` and upgrades npm to ≥ 11.5.1; npm then authenticates the short-lived GitHub Actions OIDC token against a *trusted publisher* configured per package on npm (pointing at `d-hinders/Haven-AI` + workflow `publish.yml`, **environment field empty** — see the bullet below for why that matters). Nothing to leak or rotate on this path, and it's exempt from the 2FA one-time-password prompt that blocks token-based publishes.
