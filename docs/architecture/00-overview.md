...
(#1440) — the closure sequence is in the [decision log](../archive/decision-log.md#2026-08-14--retire-the-safe-rail-entirely-1440), not repeated here.
Existing Safe rows are untouched and still readable to a direct database query,
but no account, agent or dashboard surface displays them, and Haven has no
legacy Safe payment path. The Smart Sessions **session rail is retired** too (#834; accounts still
...
Dashboard creates a pending setup for a delegation account → user runs the setup
prompt locally → connector generates both credentials locally, registers proof
and public metadata, stores credentials, and configures the runtime → user
approves the budget-delegation signature in the Haven modal, which also
activates the agent (#1069/#1076) → the runtime activates its new MCP entry → the agent makes the
...
- **Safe + AllowanceModule — RETIRED (epic #1440).** Existing Safe accounts
  ...
  exception, having no rail predicate) and the rail admits and executes nothing —
  sequence in the [decision log](../archive/decision-log.md#2026-08-14--retire-the-safe-rail-entirely-1440). It is
