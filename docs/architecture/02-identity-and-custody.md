...
> **One live rail; these zones are the retired baseline.** The zones, diagram,
> and invariants below describe the **legacy AllowanceModule rail**, which is
> **RETIRED** (#1440); the closure sequence is in the [decision log](../archive/decision-log.md#2026-08-14--retire-the-safe-rail-entirely-1440). Existing
> Safe rows are untouched and still readable to a direct database query, but no
> account, agent or dashboard surface displays them, and Haven cannot execute
> legacy agent payments from them. Any residual Safe permission remains on-chain and is managed
> externally through Safe's own interfaces by the Safe owner; Haven can no
> longer construct or relay an agent transfer on this rail. The Smart Sessions
> **session rail is retired** too (#834):
> `session_key` accounts get HTTP 410 from the payment paths. Every account that
> can spend runs on the
