...
> this diagram: the account-level READS, and `POST /safe/exec`, which accepts an
> owner-signed execution and relays it for gas only. It is a lower-level access path, not a Haven
