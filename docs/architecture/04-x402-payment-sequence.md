...
observer is leader-locked with a per-intent advisory lock; against the
agent-report path the CAS/replay/ambiguity guards in
`confirmObservedSettlement` decide the winner. #2094's intent-unique
settlement child is what makes a swept `Transfer` safely attributable to
exactly one intent — its `RedeemedDelegation` log names the payment, closing
the look-alike-twin refusal that used to keep both out of the feed. The
generic plain-HTTP erc7710 flow (#2041), where the AGENT retries the merchant
and Haven never sees the header, is covered by the same observer: the on-chain
settlement is discovered regardless of how the hash would have travelled.
