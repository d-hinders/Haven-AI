...
- **`POST /x402/resume` does not exist** because resume is consolidated
  ...
  > **`resumeX402Payment()` cannot currently complete**: its guard requires a
  > `next_action` nothing emits, so it throws rather than retrying (#2131). The
  > rehydration half — `GET /payments/{id}/resume_state` — works and is
  > unaffected. Full analysis and the tracked gap:
  > [`04-x402-payment-sequence.md` § *Resuming An Authorized Payment*](04-x402-payment-sequence.md#resuming-an-authorized-payment)
  > and [#2145](https://github.com/d-hinders/Haven-AI/issues/2145).
...
135 registered routes, **2 of them undocumented** — only safe-deploy.ts and safe-exec.ts, deliberately, under the #1440 Safe-rail retirement. (#1698's six re-key routes were documented in the same PR that added them, which is the gate working as intended: the undocumented count is shrink-only, so a new route module has nowhere to hide. Note that only the undocumented count is enforced — the TOTAL here is prose and goes stale silently with every route added, so re-count it rather than trusting it.)
