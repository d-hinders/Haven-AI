...
- **The human shape has exactly one producer.** `rails/delegation-budget-view.ts`
  ...
  carries the same value as `allowanceAmount`, typed there as a bare `string`
  (`DashboardAgentAllowance`), so on that route the human-decimal digits are
