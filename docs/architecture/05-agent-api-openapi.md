...
- **The human shape has exactly one producer.** `rails/delegation-budget-view.ts`
  ...
  what changed). Naming it does not DISCRIMINATE it: the pattern admits a bare
  integer, so on that route the human-decimal digits are
  pinned by the literal in `routes/__tests__/dashboard.test.ts` and the field
  set, types and uuid formats by its `expectMatchesSpec` round trip; the
  creation response (`POST /agents`, `allowances: []`, no view call) is
  round-tripped against `CreateAgentResponse` the same way.
