...
**An argument the tool does not declare is refused — on 19 of the 22 hosted
tools (#2312, #2348, #2349).** It began with the money-path tools that read
...
take no input and which a supported runtime decorates with a dummy key; and
`haven_complete_mcp_tool`, held for #2353's rollout) — and a tool on neither
does not compile.
