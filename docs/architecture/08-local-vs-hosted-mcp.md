...
payloadless `POST` is a different request from one with no body at all.
Conflating them would be the same class of error the refusal existed to avoid
committing. That divergence is gone rather than mitigated: one field, one name,
both surfaces.

**The spelling half is deliberately NOT done**, and the reason is the same one
that held it out of #2348: converging `idempotencyKey`/`idempotency_key` renames
an argument on the **published** `@haven_ai/mcp` package. It needs a deprecation
window — accept both, warn on the old, then drop — which is a release-train
decision rather than a parse one. It stays open on
[#2366](https://github.com/d-hinders/Haven-AI/issues/2366) awaiting that call.
The reason renaming is a
