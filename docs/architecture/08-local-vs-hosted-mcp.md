...
versus `payment_required` (hosted) is **not a spelling difference**: hosted
`payment_required` is the parsed HTTP 402 PaymentRequired object, while local
`quote` is the whole `X402Quote` and is passed to `payX402Quote` as such.
Renaming one to the other would change *what a caller must send*, not what it is
