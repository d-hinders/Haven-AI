...
verified:
  - "#2738: EDITED, scope = the `x402 over-budget reject` row only. Its evidence column said \"Record the enforcer named in the revert reason\", which #2706 made unfollowable: the ordinary refusal on that leg is now a 403 pre-check with no revert reason at all, so a tester would hunt for one that does not exist and could record its absence as a failure — the same shape as the #2016 entry below, where the old row made a correct refusal read as a failure. The row now names both refusals and the evidence each carries. The adjacent `over-budget direct refusal` row (137) is UNCHANGED and still correct: `readRemainingBudget` appears nowhere on the POST /payments path, so that leg is still the enforcer/502 one — verified by grep across packages/backend/src, five call sites, none of them payments. Found by the docs-coupling advisory on the #2738 fix, after haven-reviewer had explicitly recorded this file as one it did not read. Scope: that ONE row. NOT re-verified: the other scenario rows, the report structure, or any other section."
...
| x402 over-budget reject | Rejects with no signable intent, on the EIP-3009 funding leg (#2016). Since #2706 the ordinary refusal is HTTP 403 `delegation_budget_exceeded` — the same pre-check as the erc7710 row below; the enforcer revert is now only the FAIL-OPEN path, taken when the budget read is not from chain | pass/fail/skip | | | Record `error_code` and `remaining_atomic` for a 403 (a bare 403 is also what a MISSING delegation returns), or the enforcer named in the revert reason for a 502. **Do not hunt for a revert reason on a 403** — there is none, and its absence is not a failure | |
