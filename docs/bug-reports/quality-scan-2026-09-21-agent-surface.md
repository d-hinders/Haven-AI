...
- **The signer half was NOT the tree.** `haven-signer-qa-dev` on this
  ...
  404` and **no** `next_tool_name` — which is exactly what
  `packages/signer/src/sign-context.ts:130-135` sets and `tools.ts:771-794`
  renders at head. The live signer predates the fix; the tree carries it.
...
contacted" — the prepare tool's copy, on a read-only check; `token: "USDC",
...
with no shape (`contracts.ts:160`), `haven_get_agent`/`haven_get_allowances`
...
`packages/mcp-server/src/tools/support/errors.ts:228-246` — the
...
- N1 — the strict-refusal explanation for an unknown key is the cap
  ...
  sentence in `parsing.ts`'s copy, cosmetic until an agent is misled by it.
