...
>
> **One item does not vary with the route: the independent reviewer pass.** The rule, and why self-review does not substitute for it, are stated in [`AGENTS.md`](../../AGENTS.md) § *Agent Roles*; the verbatim 2026-08-21 owner decision behind it is in CLAUDE.md § *How shipping is governed*. Neither is repeated here (#2640). What this document adds is the consequence for the paragraph above: *owning the equivalent judgement yourself* means running `haven-reviewer` yourself, so working outside `ship-next` does not reach this item. That stays consistent with *enforce outcomes, never tooling*: `.claude/hooks/ship-next-guard.sh` gates on whether review HAPPENED, never on which workflow ran.
