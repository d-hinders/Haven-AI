...
`git worktree remove <root> --force`, never `rm -rf`, because the registration is
the half that matters and a deleted directory leaves an entry only `prune` clears.
...
exceptions, both because the tree is about to be used again: a `blocked` verdict,
and a guard refusal.
