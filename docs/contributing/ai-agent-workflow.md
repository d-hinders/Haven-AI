...
>
> **One item does not vary with the route: the independent reviewer pass.** Why self-review does not substitute for it is stated in [`AGENTS.md`](../../AGENTS.md) § *Agentic Workflow*; the rule itself, the verbatim 2026-08-21 owner decision behind it, and why the old conditional form was the problem are in CLAUDE.md § *How shipping is governed*. Neither is repeated here (#2640). What this document adds is the consequence for the paragraph above: *owning the equivalent judgement yourself* means running `haven-reviewer` yourself, so working outside `ship-next` does not reach this item. That stays consistent with *enforce outcomes, never tooling*: `.claude/hooks/ship-next-guard.sh` gates on whether review HAPPENED, never on which workflow ran.
...
- **Instrument Self-Reference And Staleness.** If a PR-body claim rests on a measurement: confirm the instrument looked at the column, the tree and the process you mean. Filter a multi-column listing to the column before counting it (`git worktree list | grep -c review` counts branch names too, and its value is machine-local), `git fetch` before any "the code is not there", and never conclude a value appears in output you passed that value to. Correcting a false claim is itself a measurement — verify the replacement against its instrument, not against the sentence it replaces (see [`session-retrospective-2026-09-06.md`](session-retrospective-2026-09-06.md)).
...
- review status, written as a **named verdict line per pass, naming the head it reviewed** —
  `haven-reviewer: passed @ <sha> | skipped because ___`, and the same for `haven-design-reviewer` on `area:frontend`
...
Use after implementation, on every diff. The coupling gate flags the docs whose `covers:` front-matter maps to the changed code, and those are the floor of what the pass reviews, not the scope — it derives that from the diff's claims (#2499). It reports specific stale, missing, or broken doc claims so the captain can update them before merge. The agent's findings are read-only suggestions, but **running it and acting on its findings (the `covers:`-mapped ones are the floor, not the scope — #2499) is a hard definition-of-done step in the loop, not optional** — the captain must update the implicated docs (or genuinely re-verify and bump `last-verified`) in the same PR before opening it. Advisory here means the **docs↔code** coupling comment does not by itself block auto-merge (contract docs excepted), not that the step can be skipped. The *design-system* coupling gate is a different check and does block — see [`autonomous-pr-loop.md`](autonomous-pr-loop.md).
...
10. Ask `haven-reviewer` for a final diff review. Every pull request, unconditionally — the risk list this step used to carry was the licence for skipping it (owner decision 2026-08-21; `CLAUDE.md` § *How shipping is governed* is canonical since #2639).
11. Ask `haven-doc-reviewer` for a doc-accuracy pass after implementation; the coupling gate flags the docs whose `covers:` front-matter maps to the changed code, and those are the floor, not the scope — the pass derives its own scope from the diff's claims (#2499). Update the implicated docs before opening the PR.
...
Three limits, stated because a guard whose limits are unwritten gets over-trusted:
...
**And the root has an end (#2601).** The pass that made it removes it —
`git worktree remove <root>` — never `rm -rf`, because the registration is the
half that matters and a deleted directory leaves an entry only `prune` clears,
and **never `--force`**, because plain `remove` refuses a root holding modified
or untracked files and that refusal is the only thing standing between the rule
and a captain-requested patch made inside the root.
That was written down after **173 worktrees** accumulated on one machine, none of
them prunable, at which point `git worktree list` stopped being readable as the
"is another session working here" signal the claim-before-build protocol leans on
— a coordination cost, not a tidiness one. Nothing but the pass itself can tell an
abandoned root from a live one, which is why this cannot be a periodic sweep. Two
exceptions where a CLEAN root still stays, both because the tree is about to be
used again: a `blocked` verdict, and a guard refusal. The dirty-root case is not
a third exception — it is enforced by git rather than remembered by anyone.
...
14. Use haven-reviewer for a final diff review. Every pull request, unconditionally (owner decision 2026-08-21; `CLAUDE.md` § *How shipping is governed* is canonical since #2639).
15. Use haven-doc-reviewer after implementation, with a document's `covers:` front-matter as the floor of its scope rather than the whole of it (#2499), and update any stale claims it identifies.
