...
>
> **One item does not vary with the route: the independent reviewer pass.** The rule and the owner decision behind it are stated once in CLAUDE.md § *How shipping is governed* and are not repeated here (#2640). What this document adds, because it is about working with agents: *owning it yourself* means running `haven-reviewer` yourself, not reviewing your own diff — the author is the one person who cannot see the assumption they already made. That stays consistent with *enforce outcomes, never tooling*: `.claude/hooks/ship-next-guard.sh` gates on whether review HAPPENED, never on which workflow ran.
