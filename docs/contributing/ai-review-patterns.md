...
- If wallet or passkey readiness is incomplete, a live delegation UI must keep the recovery action visible. A warning such as "Wallet approval unavailable" must be paired with `WalletButton`, passkey guidance, or an equivalent next action. The retired-rail exception this rule used to carve out is gone: #2413 deleted the legacy notice and stopped rendering legacy accounts, so every surface this rule reaches is a live delegation surface.
...
- Generated artifacts should explain declined-payment behavior — budget, recipient, and expiry are enforced on-chain before any money moves — and avoid implying the agent can spend beyond the user's rules or that a payment can be queued for later approval (#2063: no live rail queues; out-of-policy payments are declined during prepare).
