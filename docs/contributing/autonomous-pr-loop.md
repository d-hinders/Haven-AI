...
that is a deploy-provider configuration matter, and the evidence, the operator
fix and the freshness alarm that now reports its silence are in
[`agent-qa.md`](../operations/agent-qa.md) § *Post-deploy trigger (webhook
setup)*. Also, `git diff
