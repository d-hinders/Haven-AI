...
verified:
  - "#2767: EDITED, scope = the Merge policy A bullet's `nit` sentence only. It said a nit 'is fixed in place when it is a one-line change, or filed as a follow-up with its screenshot'; the filing path is retired — a nit is fixed in place or dropped under the PR body's **Not filed** list (#2767), and the two new body sections this PR renames the template's *Intentionally Left Out* into are named here so the routing is readable from the loop's own page. The sibling paragraph the issue asked for in `CLAUDE.md` § *How shipping is governed* is NOT in this change: that file is write-protected against this session's tooling, and the gap is disclosed in the PR body rather than papered over — `CLAUDE.md` is unedited and its claims about the loop are unaffected by this diff. No rule restated: the three dispositions live once, in `ship-next` § *Rework caps*, linked from here. #2512's lesson honoured — the new pointers were each verified to name content the target file carries at this commit. Scope: that one bullet. NOT re-verified: the ruleset inventory, the money-path safety model, the migration-review guidance, the loop constraints, or any figure in the file."
...
- A **non-money-path** PR (docs, tests, mechanical refactor, other code)
  ...
  **`should-fix`** UX, copy, or design-system finding, the loop **pauses** even if
  ...
  — it is fixed in place when it is a one-line change, or dropped with a reason under
  the PR body's **Not filed** list (#2767 retired the follow-up-issue path for it);
  the severity table and the reasoning are in that playbook, not here.
