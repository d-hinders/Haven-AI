...
the SHA deployed to dev. For a `deployment_status` post-deploy run they
coincide by construction (`GITHUB_SHA` *is* the deployed commit); for the
nightly cron, a lagging or failed dev deploy makes the run's `headSha`
overstate what was actually exercised. **Read that as one theoretical case and
one that is still the only one in practice (#2268 → #2273):** the trigger that
used to be named here, `repository_dispatch: dev-deployed`, fired 0 times in
156 runs because Railway offers no place to send it from; #2273 rebuilt it on
GitHub's own `deployment_status` event, which Railway's integration does emit.
Two limits stand until measured: it has **not yet been observed firing** (it
runs only the default branch's workflow file, so the first real deploy after
the merge is the evidence — #2268/#2273 operator-verify), and a Railway
deployment is created against a bare SHA, so the run is expected to carry no
`headBranch` — which this gate's `--branch dev` query cannot see until #2404.
Until then the reassuring half of the contrast is real for the guard and the
run history, and not yet for this gate. The evidence, the dedupe and the
provenance rule that stops a manual dispatch from impersonating a deploy
(#2271) are in [`agent-qa.md`](../operations/agent-qa.md) § *Post-deploy
trigger (`deployment_status`)*. Also, `git diff
