...
> **The `latest` tag move needs its OWN credential — OIDC does not cover it
> ([#2647](https://github.com/d-hinders/Haven-AI/issues/2647)).** Trusted
> Publishing authorises `npm publish` and nothing else, so `npm dist-tag add`
> gets no token from it. Production run 34114664962 published all five packages
> under `alpha` and then failed every `latest` move with E401 — deterministic,
> not a flake, and a retry does not fix it.
>
> `promote_latest()` therefore reads `secrets.NPM_LATEST_TAG_TOKEN` and refuses,
> loudly and by name, when it is unset. **Provisioning that secret is an
> operator step this repository cannot perform**, and until it is done every
> promotion that bumps a version will publish correctly and leave `latest`
> behind.
>
> **Treat that token as publish-capable.** npm's granular tokens are read-only
> or read-and-write over a package set; there is no verb-level scope that
> allows a dist-tag move while denying a publish. So the token an operator
> provisions here CAN publish these five packages, whatever the workflow uses
> it for — store, rotate and audit it accordingly. What the workflow guarantees
> is narrower and is a property of the wiring: the token is applied to that one
> command through a throwaway `--userconfig`, never to a publish call.
>
> **Re-run the NEWEST failed publish, not an older one.** Since #2647 a package
> already on npm still gets its `latest` healed rather than being skipped, which
> is what makes a re-run a recovery. `promote_latest()` refuses a move that
> would take `latest` *backwards*, so re-running a superseded run fails safely
> instead of dragging the tag down — but the run you want is the most recent
> one.
...
| `publish.yml` | push to `main` | **prod channel**: publishes packages whose version isn't yet on npm, under the tag its version implies (`alpha`/`latest`), then moves `latest` onto each one it published (#2536) so a bare `npm install`/`npx` gets the newest release |
