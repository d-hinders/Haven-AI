...
verified:
  - "#2767: re-verified, NOT edited. The strict coupling gate implicated this doc (its `covers:` names `.github/workflows/promotion-digest.yml`), and its claims about that workflow were each re-read against the file as this PR changes it: § *What's in prod vs. pending* (refreshed on every promotion, weekly, on-demand; the one-long-lived-issue paragraph, whose upsert the workflow does by listing under the `promotion` label and editing the number that returns — read against the `gh issue list --label promotion` / `gh issue edit` pair at this head), and the § Workflows table row 'upserts the pending-promotion issue'. All survive untouched: this PR adds an advisory metrics block to the digest BODY only — no trigger, no upsert key, no label, no lifecycle change. Scope: those three passages. NOT re-verified: the release/publish rows, the promotion checklist, the branch model, or the rest of the file."
