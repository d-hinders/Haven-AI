...
> GitHub before parsing, while the guard's regex sees only the raw bytes; and a
> keyword straddling the 70th character of a commit subject came back from
> `gh pr view --json commits` split mid-token, so the guard's first reading
> could not see it.
> All three were measured in #2337, where the guard grew its commit-message and
> title readers. Bare is the one form GitHub and the guard read identically. The
