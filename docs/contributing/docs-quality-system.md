...
Of the three hits at saturation:

- **One real defect.** `docs/product/agent-key-rotation.md`, entries `#1849` and
  `#1702` truncated mid-sentence during a merge resolution. The reason the check
  exists.
- **Two pre-convention notes**, reported as `(entry)` because they have no
  leading `#ref`: `docs/quality/scan-ledger.md` and
  `docs/contributing/branch-and-release-flow.md`, both from before the `Prior:`
  chain convention (#1496), when a new note REPLACED the old one instead of
  chaining it. `chain-integrity-backtest.mjs`'s own header records the identical
  shape for `checkChain` — 40 historical failures, all of them pre-#1496 — and
  the answer is the same: the check is not retroactive. It only ever compares a
  pull request's own edit against that pull request's own merge base, so
  history written under the older convention is replayed, never gated.
