...
The tool no longer misreports that as progress: a doc that leaves the governed
set is named in a **"no longer governed"** list, and is excluded from the
"residue shrank" hint that would otherwise invite an `--update` discarding those
gaps permanently.
