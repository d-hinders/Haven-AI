...
```bash
npm run docs:check   # front-matter + covers globs, agent skills, README agent section, last-verified chains
...
`scripts/docs/chain-integrity.mjs` is the fourth `docs:check` step and is
...
All four blocking scripts need no npm dependencies and finish in seconds, which is why the
