...
It is **phase-gated**, and that is not a detail. `next start` loads the config
too — measured, not assumed: deleting the output and starting the server
without rebuilding put the files back. This app builds with
`output: 'standalone'`, where the deployed artifact need not contain the
repository's `docs/` tree at all, so an unguarded call would throw on a missing
source and take the server down on boot — strictly worse than the 404 it
prevents. Build and dev generate; a production server does not.
...
| Chain size band, 40 KiB ([#2562](https://github.com/d-hinders/Haven-AI/issues/2562)) | `scripts/docs/chain-integrity.mjs` (same run) | Advisory — printed for every governed doc, blocks nothing |
...
**Retention: compact at the band, in its own pull request ([#2562](https://github.com/d-hinders/Haven-AI/issues/2562)).**
Chains grow monotonically — every verification prepends, nothing retires — and
until #2562 nothing said when to stop. Two things follow from that, and the
first is why the second exists:

- **The ceiling is a hard stop with no warning, and it is diff-scoped.** The
  runner skips a doc whose `last-verified` line is unchanged, so a chain crosses
  64 KiB **silently** and the cost lands on whoever next edits that doc for an
  unrelated reason. That is not hypothetical: #2557 was a TypeScript interface
  fix, the coupling gate required a note on `mcp-runtime-compatibility.md`, and
  a normal-length note pushed the line over. The person who trips it is the
  least-equipped to fix it — compaction is explicitly a human decision, and they
  have no context on which of 50-odd entries still matter.
- **So a non-blocking band at 40 KiB is reported for every governed doc on every
  run**, changed or not, ahead of the promotion/push/no-base early returns. It
  blocks nothing, at the band or above it: the ceiling stays the only hard stop
  and stays diff-scoped, because failing a pull request over the size of a doc
  it never touched is the same ambush one layer up.

**When your run prints a band warning**, compact that chain in a pull request of
its own — front matter only, `Refs` the tracking issue, the shape
[#2563](https://github.com/d-hinders/Haven-AI/pull/2563) used:
