...
merged history rather than by argument (`chain-integrity-backtest.mjs`, 169 pull
requests and 256 changed chain lines since 2026-08-15): the only three hits were
one deleted full stop against two real defects — a prior entry truncated
mid-sentence, and an entry deleted outright whose ref survived as a citation
inside another entry, which the ref check reports as healthy by construction. So
whitespace and a single terminal period are ignored; any change to a word is a
finding. A declared `chain-reset(#N)` is exempt, since a compaction rewrites
entries on purpose.
