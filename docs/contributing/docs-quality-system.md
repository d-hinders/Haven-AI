...
The baseline (`scripts/docs/covers-gaps-baseline.json`) is **shrink-only**, in
the same house style as `ui-gate-wording-baseline.json`: doc → gap count, counts
may fall and never rise, `--update` rewrites it for a reviewed, intentional
change only. Seeded at **129 pairs across 40 of 73 governed docs**.

**What the extraction does not catch** is listed in the script's header and
pinned by `covers-gaps.test.mjs` — a path split across a hard wrap or by inline
markup, a path outside the three prefixes, a path with no code extension, a
package-relative path, and a file that no longer exists. The count is a floor,
not an inventory, and the baseline must not be read as completeness.
