...
merged history rather than by argument (`chain-integrity-backtest.mjs`, measured
at `f9d920fa`: 169 pull requests, 256 changed chain lines since 2026-08-15).
The replay produced three hits, and only one of them was a defect in a doc:

- **A prior entry truncated mid-sentence** (`docs/product/agent-key-rotation.md`,
  entries `#1849` and `#1702`). The real thing, and the reason the check exists.
- **A deleted full stop** (`docs/operations/mcp-runtime-compatibility.md`). Noise
  — a gate that goes red over punctuation teaches people to route around it, so
...
After both, the same replay reports **one** hit: the real defect. A declared
`chain-reset(#N)` is exempt, since a compaction rewrites entries on purpose.
