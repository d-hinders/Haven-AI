...
verified:
  - "#2767: EDITED, scope = the Guidelines table's C1 and C3 rows only, and the C3 row's `Landed in` column. C1 and C3 each gain one sentence placing the instrument's residue in the PR that triggered it — C1: 'The sweep is run and its residue finished in the PR that triggered it — not filed as a follow-up (#2767)'; C3: 'The widened net is swept and finished in the PR that triggered it, never filed as a sibling (#2767)'. C1's `Landed in` now names the three homes the rule actually has at this commit — `ship-next` *Implement* item 6 (the sweep itself), the *Rework caps* non-converging exit + **Filing bar** check 3 (finish it here), `doc-reviewer` §2 — each verified present on the branch before being named, which is what #2512 exists for. C3's names `ship-next` *Implement* (#2512) plus **Filing bar** check 5, and its record that the rule was absent when this document was written is preserved untouched. No figure in this document is moved by this change: the tables, the eight patterns, the census and the per-category counts stand on the entries below. Scope: those two rows and the one column. NOT re-derived in this pass: any figure, patterns 1-8, rows A/B/C2/D/E/F/G/H, or the merge-method census."
