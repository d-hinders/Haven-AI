...
| **E2** | Refresh a base by interleaving `last-verified` chains, never by taking a side, and assert the result on raw bytes. | Retired with the chain (#2681, 2026-09-09): `verified:` blocks no longer exist to interleave, and `chain-integrity.mjs` is deleted; the history lives in `docs/archive/last-verified-chains-2026-09.md`. Kept here as the record of what E2 answered. |
