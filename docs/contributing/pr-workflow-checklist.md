...
| Backend/API | `npm run typecheck -w packages/backend`, `npm run test -w packages/backend`, `npm run lint:deps` (dependency boundaries, #982), `npm run lint:db-mocks` (positional DB-mock ratchet, #1227), `npm run lint:retired-rail-prose` (retired-rail prose ratchet, #2685 — scans all of `packages/**`, not just the backend), and `npm run lint:request-schemas` (request-schema ratchet, #3029 — shrink-only over the request-validation rollout, epic #3028), `npm run lint:next-steps` (typed next-step ratchet, #3104 — scans `packages/mcp-server`, `packages/signer` and `packages/mcp`, and runs in their jobs as well as here), and `npm run lint:vocabulary` (MCP↔CLI vocabulary guard, #3131 — adding a property to the OpenAPI transaction base fails CI until the field is declared in the vocabulary map beside the guard; it runs in **Repo CI config checks**, not *Backend checks*, because its other input is on the SDK receipt surface) |
