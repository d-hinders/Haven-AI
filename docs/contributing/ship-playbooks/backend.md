...
- **Money path.** If the change touches `routes/x402.ts`, `routes/payments.ts`, `routes/machine-payments.ts`, `lib/{machine-payments,payment-coverage,allowance-module}.ts`, `middleware/agentAuth.ts`, or `db/migrations/`, also load [`money.md`](money.md) — characterization tests first, human merge gate.
