...
For other files in `casp-risk-guardrails.md`'s `covers:` list (e.g. the passkeys / safe-deploy / user-accounts routes — `infra/relayer.ts` was on this list of examples until #1607 put the relayer globs on the money-path list itself), the §1 required reading still applies — §2 scopes only the characterization-test requirement. **The regulatory `covers:` list is deliberately wider than the money-path list and is not the same question**; do not treat a difference between them as drift — with one exception since #3098: a *package-wide* `covers:` entry (`packages/<pkg>/**`, `packages/<pkg>/src/**`) must be on the money-path list or named `DOC_ONLY` in `scripts/ci/money-path.test.mjs` with its reason. File-level and sub-package differences remain expected and are not drift. Worth knowing which way that asymmetry runs, though: `casp-risk-guardrails.md` has covered `routes/agent-rekey.ts`, `modules/agents/rekey-*.ts` and `infra/repositories/agent-rekeys.ts` since #1698 (PR #1762), while the money-path list carried none of them until #1892 — the regulatory perimeter was complete and the classification perimeter was not, so the wider list is a useful place to look when adding to the narrower one.
