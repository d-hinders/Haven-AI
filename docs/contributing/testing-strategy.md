...
and three different ids. At 63 real-DB files that is 63 full migration runs per
...
ordinary run reuses; the tail is dead weight). It **refuses any non-local host**
with no override flag, because it drops schemas and "the operator passed the
wrong URL" is exactly the case a destructive script has to survive.
