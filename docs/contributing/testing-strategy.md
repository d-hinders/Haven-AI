...
Both harness entry points can pay the **cold** cost: `resetDb()` awaits
`initDbHarness()`, which brings the worker's schema to the migration head and
serialises that run across workers on one advisory lock, so a waiting worker
pays the runs queued ahead of it too. `vitest.config.ts` budgets exactly that
...
suite had not grown, and what moved was execution under contention. Forty-eight
backend test files put the harness in a hook and could not trip it. The two that
did not, did: `uuid-param-22p02.test.ts` and `catalog-ingest-lock.test.ts` timed
out on pull requests they had nothing to do with, twice
...
1. the file also calls the harness from `beforeAll`/`beforeEach`, so the cold run
   is already paid and the in-body call is a warm one (~25 ms) — this is how the
   harness's own suites reset mid-test, where the reset **is** the subject; or
2. that `it` declares an explicit timeout of its own, as
   `db-harness-lock-concurrency.test.ts` does at 180 000 ms to keep an unwarmed
