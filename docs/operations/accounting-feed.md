...
| **PRE-push** | the supplier lookup/create or the **invoice POST itself** answered 403 / `[2000663]` — **nothing exists in Fortnox** | `skipped`, `error = scope refused before the invoice was created: Fortnox POST /supplierinvoices failed (HTTP 400: … [2000663]).` — re-claimable | `scope_missing`, `missing scopes: supplierinvoice — …` |
