...
| Login with a return target | `/login?next=<same-origin path>&via=agent` | Same sanitiser, same helper. |
...
| Budget approval for a setup | `/agents?setup=<setupId>` | Opens the connect modal on the step that setup's live status calls for. The canonical form is `approval_url` from setup create **and** status — print that, never a hand-assembled URL. |
