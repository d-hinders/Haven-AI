...
| capability manifest | `/.well-known/haven.json` | **Generated, never hand-edited.** The same environment as JSON for an agent's code. Its environment-dependent half comes from the backend's `GET /discovery`, so a connector-channel or hosted-MCP change propagates instead of being restated. Adding a key is non-breaking; removing one or changing its meaning bumps `schema_version`. `environment` is `production` or the deployment's own name (`dev`), read through `packages/frontend/src/lib/env.ts` — the same helper the `DEV` badge uses — never `unknown` (#2709): production leaves `NEXT_PUBLIC_HAVEN_ENV` unset by convention, and the manifest reports the convention, not the raw variable. Each `chains.supported` entry carries `testnet`, derived from the registry's faucet rule (#2534), because production lists a testnet beside its mainnets and `environment` alone cannot say which chain carries real money. `chains.default` is the expected deployment chain only; after login, confirm the funding chain with `haven wallets funding`. `api.openapi_mirror` says the dashboard-relative `/api/openapi.json` alternate is the same OpenAPI document as `api.openapi`. |
| discovery facts | `GET /discovery` (backend) | Public, read-only, unauthenticated. Re-serves values that are already public elsewhere — never per-user or per-agent data, no relayer address, nothing from `/health`. It carries the canonical `chains.default` alongside supported/deployable sets; the backend root document points to the configured frontend manifest for callers handed only the API host. Tests pin the response shape and configured-origin rule. |
...
the nomination helper refuses to nominate one for `latest`. The move is tied to
a publish this run actually performed, so re-running an old workflow run (which
publishes nothing, every version being on npm already) cannot drag `latest`
backwards. It does not compare against the live `latest`.

**#2647 moved that mechanism into a second job, and the first attempt never
worked at all.** #2536 put `npm dist-tag add` inline in the publish job, which
authenticates by npm Trusted Publishing (OIDC) — a credential that authorises
`npm publish` and nothing else. The 0.1.35-alpha.0 release was its first real
execution: all five packages published under `alpha`, all five tag moves failed
E401, and `latest` sat at 0.1.34-alpha.0 — the very state this decision exists
to end, produced by the mechanism meant to end it. Since
[#2647](https://github.com/d-hinders/Haven-AI/issues/2647) the publish job only
**nominates** (`record_latest_promotion()`, which is what the walls above now
refuse in), and a separate `main`-only **`promote-tags`** job performs the move
with a granular npm token held by the `npm-production-tags` GitHub Environment.
Three things a reader of this section needs from that:

- **The decision is unchanged**; only the plumbing under it moved.
- **A promotion can be half green** — packages live, `latest` unmoved — so
  verifying a release means reading two job results, not one, and `npm view` is
  the evidence either way.
- **There is now a long-lived credential with an expiry.** npm caps
  write-enabled granular tokens at 90 days; this one lapses **2026-12-06**, and
  when it does, releases publish and `latest` silently stops moving. The
  durable answer is to stop needing two dist-tags — publish prereleases under
  `latest` directly, which needs no second mutation and no credential — and it
  freezes `@alpha` for anyone pinned to it, so it is a product decision that
  wants making before the expiry rather than on the day a release breaks.

**#2580 closed that gap one step earlier, and the decision is option C.** Rather
than compare against the registry at publish time, `scripts/release-bump.mjs`
now refuses a bump that does not move the version **forward**
(`release-version-order.mjs`). It fails at the release PR, where a human is
already looking and the remedy is to type a different number — instead of after
publication, where the remedy is a manual dist-tag repair.

Two things about that choice, both deliberate:

- **The baseline is the repo's current version, not the published `latest`**,
  which is a narrowing of how #2580 worded the option. This script is offline,
...
- **Dev-channel snapshots are exempt.** `0.0.0-dev.*` sorts below every real
  ...
  guard and caught by execution; it has its own test here.

Option A (a `semver` comparison inside `publish.yml`) stays rejected on the
measurements in #2580: `sort -V` is not semver — it orders `1.0.0` *before*
`1.0.0-alpha.1`, so it would refuse the 1.0 release this repo is heading toward
— and `semver` is not a direct dependency. Option C sidesteps both: the bump
script already resolves `semver`, and a failure there is a loud local error
