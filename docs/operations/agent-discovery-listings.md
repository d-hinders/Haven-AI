...
| capability manifest | `/.well-known/haven.json` | **Generated, never hand-edited.** The same environment as JSON for an agent's code. Its environment-dependent half comes from the backend's `GET /discovery`, so a connector-channel or hosted-MCP change propagates instead of being restated. Adding a key is non-breaking; removing one or changing its meaning bumps `schema_version`. `environment` is `production` or the deployment's own name (`dev`), read through `packages/frontend/src/lib/env.ts` — the same helper the `DEV` badge uses — never `unknown` (#2709): production leaves `NEXT_PUBLIC_HAVEN_ENV` unset by convention, and the manifest reports the convention, not the raw variable. Each `chains.supported` entry carries `testnet`, derived from the registry's faucet rule (#2534), because production lists a testnet beside its mainnets and `environment` alone cannot say which chain carries real money. `chains.default` is the expected deployment chain only; after login, confirm the funding chain with `haven wallets funding`. `api.openapi_mirror` says the dashboard-relative `/api/openapi.json` alternate is the same OpenAPI document as `api.openapi`. Since #3304 each `packages` entry also carries its release record (`released_version`, `recommended_version`, `min_version`, `upgrade_command`, `notes`), built from `@haven_ai/core` so it still answers when the backend is down, with `min_version` / `recommended_version` taken from the backend's `client_releases` when it is reachable (the two deploy separately), and `release_notes_url` names `/releases`. |
| releases page | `/releases` | **Generated per request** (`src/app/releases/page.tsx`) from the same `buildReleaseCompat` data the capability manifest (above) and `GET /discovery` (below) serve, with the thresholds taken from the reachable backend: per published package, the version released in source, the minimum and recommended versions from `CLIENT_COMPAT` (the table the backend enforces), the update command on this deployment's channel, and short notes (#3304). Every `client_update` hint names it as `notes_url`, and so do both documents (`release_notes_url`). An update command is omitted, never guessed, when the backend is unreachable. The notes are generated by `npm run release:bump` from the five package CHANGELOGs into the client release data in `@haven_ai/core` (#3305); change a note in its CHANGELOG, never in the file. |
| discovery facts | `GET /discovery` (backend) | Public, read-only, unauthenticated. Re-serves values that are already public elsewhere — never per-user or per-agent data, no relayer address, nothing from `/health`. It carries the canonical `chains.default` alongside supported/deployable sets; the backend root document points to the configured frontend manifest for callers handed only the API host. Since #3304 it also carries `client_releases`, the per-package release record plus an absolute `release_notes_url`. That is public elsewhere too: the notes are the published CHANGELOGs, and the thresholds are what every `client_update` hint already tells the client it names. Tests pin the response shape and configured-origin rule. |
