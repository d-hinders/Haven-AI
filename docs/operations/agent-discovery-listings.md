...
DISPLAYED origin is still request-derived, which is a different thing: a string
echoed to the caller who sent the header, with `must-revalidate` and a `Vary`,
the same posture `robots.txt` and `sitemap.xml` take.
