...
- **eip3009 (fallback shape — a merchant not advertising erc7710, or a hosted
  deploy predating #1547):** unchanged — a compact preflight, signing by
  `payment_id` via `haven_sign_x402` (#1355 — `payment_required` rides along as
  the pre-#1355 fallback), and settling with `payment_id` + `signature` +
