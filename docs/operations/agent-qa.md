...
> (or connect with the exact setup command the dev backend's own setup response returns — its `connector_command` / `connector_package`, which since #2422 names the dev backend's configured channel and is NOT necessarily `@alpha`):
