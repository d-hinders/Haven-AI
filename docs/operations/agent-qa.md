...
1. every added and removed line matches one of three shapes — `export const
   SIGNER_VERSION = '<semver>'` (named literally, not `*_VERSION`), a package
   `"version"` field, or an internal `"@haven_ai/*"` dep pin; **and**
...
`packages/signer/package.json` — both money-path — and always *after* the last
...
**What is left is the sender**, and it is one of two things that cannot be told
apart from inside this repository: the hook above was never added to the Railway
dev backend / Vercel dev project, or it exists with a `GH_DISPATCH_TOKEN` that
lacks `Actions: write` (or has expired), in which case the `curl` fails at the
deploy provider and GitHub is never contacted at all. A dispatch sent with a
*wrong* `event_type` is a third variant with the same signature — GitHub answers
`204` and starts nothing — but "never once, for any event type, in the repo's
whole history" makes "never configured" much the likelier reading.

**Fixing it is an operator action, not a code change.** Add the hook above in the
deploy provider's dashboard, then confirm with the `gh api … dispatches` command,
which must start a `qa-dev.yml` run within seconds. Never paste the token value
into an issue, a PR or a log.
