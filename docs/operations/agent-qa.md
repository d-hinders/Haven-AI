...
- **which runs count** ([#2404](https://github.com/d-hinders/Haven-AI/issues/2404)):
  ...
  branch label — for the two events that carry one — is still `dev`, and whose
  **`money-flow` job concluded `success`** according to the jobs API. Newest
  wins; at the same commit a `deployment_status` run is preferred over a
  `schedule` over a `workflow_dispatch`, as a tie-break only. The query used to
  be `--branch dev`, which cannot see a post-deploy run at all: Railway creates
  its Deployments against a bare SHA, GitHub documents `GITHUB_REF` as "empty if
  commit" for that case, so the run carries no branch label. Ancestry is what
  "on dev" was trying to say, and it holds for a run with no label. The job
...
deployed commit — which is why the gate admits it (#2404). No such run has been
observed yet (see *Post-deploy trigger*), so that half is what the docs predict,
not what a run has shown.
...
2. **What GitHub records as the run's `headBranch`.** Railway creates its
   ...
   expected to carry no branch. The promotion gate no longer depends on it:
   since #2404 (PR #2409, landed in parallel with this change)
   ...
   `--branch dev` — see *Automation & gating* above — so it admits a
   post-deploy run whichever label the first one turns out to carry. The label
   is still read off the first run, for the record.
