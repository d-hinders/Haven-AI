...
verified:
  - "#2767: EDITED, scope = § *Automated failure reporting* only. The section said qa-dev.yml 'files a GitHub issue' on failure and closed the loop with 'close the qa-failure issue once green' — together those two halves were the defect: the workflow's own upsert only looked for an OPEN issue, triage closed each one once green, so every close guaranteed the next failure filed a fresh dated ticket (measured: 12 dated qa-failure issues created 2026-09-01..08 via `gh issue list --label qa-failure --state all --search created:>=2026-09-01`). The section now describes the standing tracker the workflow was changed to maintain — one issue, title-keyed across all states, reopened rather than duplicated — naming #2769 as its current number, and keeps the close-on-green step with its reopen semantics stated. Re-read against `.github/workflows/qa-dev.yml` on this branch (the `File (or update) the standing failure issue` step), and its reopen path verified live on #2769 on 2026-09-08: closed, the extracted step re-ran, #2769 came back OPEN with zero new issues and the open qa-failure count steady at 1. The §'s two triage routes (re-dispatch a flake / open a bug-reports/ report) and the freshness-gate cross-reference are unchanged. Scope: that ONE section. NOT re-verified: the scenario table, the preflight/funding/env tables, seeding, the trigger/gating/freshness sections, the troubleshooting entries, or the #2738/#2594 rows below."
...
On failure, `qa-dev.yml` records it in the ONE standing issue labeled
**`qa-failure`** ("qa-dev money-flow: standing failure tracker", currently
#2769) — title-keyed across all states, so a later failure reopens it and lands
as a body edit plus a comment with the run URL and trigger, never as a new
issue (#2767; the old upsert keyed on open-state while triage closed each issue
once green, which filed 12 dated tickets in the 8 days to 2026-09-08). Triage it
via [Troubleshooting](#troubleshooting): re-dispatch to clear a transient
testnet/RPC flake, or open a [`bug-reports/`](../bug-reports/) report for a real
regression, then close the standing issue once green — the next failure reopens
