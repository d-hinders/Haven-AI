...
- **Boot-time migrations, since [#2150](https://github.com/d-hinders/Haven-AI/issues/2150).**
  ...
  *normal* outcome of two replicas booting into the same non-transactional
  ...
  The polled loop itself lives in `db/advisory-lock.ts`, not in the runner
  (#2198). The real-DB test harness's cross-worker migration lock
  (`811000061`) had the identical hazard for the identical reason and now
  shares that one loop; the tuning is deliberately NOT shared, because a
  replica waiting once per boot and a vitest worker paying the interval on
  every test file want different numbers. Two hand-rolled copies that must
  stay in agreement is how this defect returns.
...
2. **Single point of stall — self-healing on the queue lane, with one named
   ...
   **lane stays blocked until an operator intervenes**. That is the accepted
   ...
   That same-nonce **cancel** (a 0-value self-send) is now ENCODED
   ([#1743](https://github.com/d-hinders/Haven-AI/issues/1743), Option A):
   `npm run ops:cancel-stuck-attest -w packages/backend -- <row-id>` runs it
   through the outbound pipeline (`infra/outbound-lane-cancel.ts`) — durable
   record, stamp-before-broadcast at the stuck nonce, fail-closed refusals for
   anything that is not really the wedge. The trigger stays **human** by owner
   decision: no timer declares an attest dead; the operator judges, the system
   executes, and either race outcome (attest mines late, or the cancel burns
   the nonce) resolves through the existing worker/probe/sweep machinery with
   no further action.
