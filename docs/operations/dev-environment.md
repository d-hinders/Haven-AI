...
verified:
  - "#2767: re-verified, not edited, scope = every claim this doc makes about `.github/workflows/qa-dev.yml` (the hosted-MCP leg turning `qa-dev` red, the freshness gate's green-run requirement, `QA_REQUIRE_ALL_LEGS=1` and the Coverage completeness step, the `deployment_status` trigger, RPC rate limits arriving as scenario failures). #2767 changed only the workflow's FAILURE step — it now upserts one standing `qa-failure` issue through `scripts/ci/qa-failure-issue.mjs` instead of filing `qa-dev money-flow failed (<date>)` per day — and this doc never described that step, so nothing here is stale. Implicated by the BLOCKING half of the coupling gate. NOT re-verified: the topology, the branch-to-deploy mapping, the configuration table, or the inspection section."
