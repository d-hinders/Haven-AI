...
- **Two Base Sepolia RPCs, and they must stay two** (#2511). The backend
  ...
  **Before either variable (or a `*_FALLBACK`) points at a new endpoint, run
  `node scripts/ci/rpc-conformance.mjs --url "$CANDIDATE_URL"` (#3336) — a
  required step.** Every line must be ✓: a JSON-RPC batch of 10 (the harness's
  ...
  plan, the September 2026 dev primary, fails the first two.
