...
verified:
  - "#2767: re-verified, NOT edited. The strict coupling gate flagged this contract doc because the PR edits `.github/workflows/qa-dev.yml` (named in `covers:`), so the four claims this file makes about that workflow were re-read against the file at this head: the hosted-MCP leg's skip reporting via the blocking Coverage completeness step (§ the `QA_HOSTED_MCP_URL` bullet — untouched, the PR edits only the `failure()` issue-upsert step), the freshness gate's dependence on a green `money-flow` job, the skip-is-failure posture (#1044/#1066), and the `deployment_status` trigger account (#2268→#2273, first observed 2026-09-02). Positive control against a rubber-stamped zero: grepping the changed hunks for `coverage`/`gate`/`trigger`/`conclusion` returns nothing while the same patterns match the unchanged steps of the same file — the instrument can say yes. The edit is confined to the failure-path issue bookkeeping (dated tickets -> one standing tracker, verified live on #2769); no gate, trigger, leg, or freshness claim moves. Scope: those four passages. NOT re-verified: the topology table, the Railway mapping beyond the claims above, the isolation rules, the inspection section, or the residuals list."
