...
verified:
  - "#2738: EDITED, scope = the over-budget clause of the edge-case note under § the x402 purchase steps. It attributed the EIP-3009 leg's refusal to an on-chain gas-estimation revert and the 403 pre-check to erc7710 alone; #2706 extended that pre-check to the 3009 leg, so the revert is now only its FAIL-OPEN path. Step 1 sends an operator here specifically to learn which refusal each path gives, so the stale clause sent them hunting for a revert reason that will essentially never appear — the same failure this file's #2140 entry records for #419 step 6, one issue later and in the opposite direction. The inherited phrase \"before any chain call\" is also tightened to \"before any redemption is prepared\": the pre-check IS a chain call — it reads the enforcer's storage — and what it precedes is the prepared redemption and its bundler estimation. Kept accurate here rather than propagated to a second scheme. Found by haven-reviewer on the #2738 fix; docs:coupling also lists this file on that diff. Scope: that ONE clause. NOT re-verified: the UI/receipt steps, the sweep-recovery note, or any other section."
...
payments, and a fail-fast remaining-budget pre-check returning HTTP 403
`delegation_budget_exceeded` before any redemption is prepared, on **both** x402
schemes —
erc7710 since #2082, and the EIP-3009 leg since #2706. On the 3009 leg the
gas-estimation revert is now only the FAIL-OPEN path, taken when the budget read
is not from chain, so do not go hunting for a revert reason there: the typed 403
is what you will ordinarily see), `PRICE_EXCEEDS_MAX`, and a merchant that
