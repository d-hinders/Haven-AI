...
to test that pairing before a production release rather than after one.
...
- **The affected tools are a declared list — and since #2349 that list is
  19 of the 22.** It is `STRICT_INPUT_TOOLS` in
  ...
  argument spelling, and #2349 closed it with the remaining twelve. The three
  that still strip are on their own list beside it, `PERMISSIVE_INPUT_TOOLS`,
  each with its reason: `haven_get_agent` and `haven_get_allowances` (schema
  `{}` — the handlers read no input, and a supported runtime decorates
  no-argument calls with a dummy key, so a refusal there would protect
  nothing and break a real client) and `haven_complete_mcp_tool` (rollout of
  the corrected `SKILL.md`, #2353). A tool on neither list does not compile.
