...
also additive, `superseded_agents_retired_locally` on a run that replaced
existing wiring, and `error.superseded_agent_ids` / `error.suggested_name` on a
`wiring_collision` refusal. It is
