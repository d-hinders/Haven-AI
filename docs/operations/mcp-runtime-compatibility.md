...
**The OpenClaw row needs a published connector.** Like every snippet row it
carries `--runtime openclaw`, and `npx @haven_ai/connect@alpha` resolves to
whatever is on npm — so an `@haven_ai/connect` release carrying the
`runtime-registry.ts` alias must reach npm before the row works end to end.
Until then the last rule above fires: the connector refuses before any side
effect, which is the correct behaviour for an id it does not know. Since #1719
that refusal is `runtime_unrecognized` and names the values it does know,
rather than reporting that no runtime was given when one was. Note what that
requires — a Railway/Vercel deploy is NOT enough, because `publish.yml` skips
any version already on npm, so the alias ships only behind a
`npm run release:bump`. The alias resolves `openclaw` to the `other` profile,
because "credentials on disk, no auto-written config, paste the snippet" IS
the OpenClaw flow.
