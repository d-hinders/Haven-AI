...
verified:
  - "#2724: EDITED, scope = ONE new checklist item at the end of § *Before opening the promotion PR* — re-measure the release scope with `npm run release:scope` and amend the CASP shard when it disagrees, plus `scripts/release-scope.mjs` added to `covers:` because this item now makes four behavioural claims about that script (its default range, that it reads sourcemaps and `files`, and both of its refusals) and nothing would otherwise re-implicate this doc when they change. THE FACT: tarballs are rebuilt from `main`'s tree AT PROMOTION TIME while the release record is written at bump time against a different tree, so anything merged to `dev` in between publishes unrecorded. Verified against `.github/workflows/publish.yml`, which deletes and rebuilds all five dist directories after checking out the push ref. An earlier draft of this note claimed the fact was stated \"nowhere in this file or any other\"; that was FALSE and haven-doc-reviewer caught it — it is stated in `docs/regulatory/casp-changelog/2026-09-07-0.1.36-alpha.0-release.md` and again, line-wrapped, in the `2026-09-07-2647-runbook-seam.md` shard (a line-oriented grep misses the second; sweep multiline). What holds is the narrower claim: zero hits across the four files a release author actually reads while cutting one — this file, `scripts/README.md`, `branch-and-release-flow.md` and the release skill — so the fact existed only as retrospective prose in two shards nobody opens at the door. Measured on the 0.1.36-alpha.0 range (783300eb..0e80a26e): 17 files +194/-41, re-derived independently with a plain `git diff --numstat` over that file list and confirmed by both review passes, against a hand count of 14 files +186/-33 that omitted connect/mcp/signer package.json while including cli/sdk. The amendment COUNT is deliberately not restated here — it already stands at three different values across three records; the 0.1.36-alpha.0 shard is dated and immutable and owns it. Scope: that ONE checklist item, the `covers:` line and this note. NOT re-verified: the npm/`promote-tags` item, the required-checks list, the migration items, the RPC variable item, or § *Run the prod smoke on the right hostname*."
