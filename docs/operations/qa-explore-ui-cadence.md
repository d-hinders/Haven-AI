...
4. **File genuinely-new findings with [`/new-task`](../../.agents/skills/new-task/SKILL.md)** — backlog-only (no `code-quality`), labeled `area:frontend` (add `money-path` only if the finding is on a money-movement surface, which then routes it through the money gate, not straight to auto-merge). Let `new-task` write the Scope / Acceptance / Files / Surface fields; seed it from the report row (surface, expected vs actual, evidence).
