...
2. **Agents receive narrow authority, not unrestricted wallet access.** On live delegation accounts that authority is a signed budget delegation enforced on-chain. Legacy Safe accounts are no longer rendered in Haven at all (#2413 — the rows persist in the database but reach no screen), and their agent-payment rail is retired: Haven does not create new legacy agent authority, mediate legacy Safe funding, or revoke those old permissions. An agent-held signing key may also control a temporary, pre-existing, or residual agent-wallet balance, so recovery and sweep controls must remain clear.
...
- Review generated credential and agent-handoff artifacts when relevant, then
  ...
  residual risk.
- Run `haven-doc-reviewer` after implementation; a document's `covers:` mapping is
  the floor of its scope, not the whole of it (#2499). Resolve stale claims before
  opening the PR.
