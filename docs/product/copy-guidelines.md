...
verified:
  - "#2728: § Enforcement's *Ratcheting baseline* bullet corrected. It told you to run `npm run lint:copy:update` --- with reviewer sign-off --- for a genuinely intentional ADDITION. That was never safe advice: the command had no refusal at all (`if (update) { writeBaseline(...); return }`, no comparison against the existing baseline), so the command this page pointed you to for a reviewed addition was also the one that absorbed an unreviewed one silently, and `npm run lint:copy` sent every failure straight to it. Measured before the fix, non-empty baseline and copy grown by one: plain run exit 1 naming the term, `--update` exit 0 writing the growth in. The three sibling ratchets on the same `lib/ratchet.mjs` had all refused to raise since they were written; this one never did. `--update` now routes through `updateRefusals`, which moved OUT of `lint-wire-types.mjs` into `lib/ratchet.mjs` so all four gates share one copy of the decision rather than three copies and a hole. Mutation-proven twice: neutering the new refusal reddens exactly the new CLI case, and making the shared helper always return `[]` reddens four tests across two guards --- so the helper is load-bearing for both, not a rename. The first-run allowance (empty baseline) is unchanged and now pinned by its own case. Scope: that ONE bullet. Nothing else on this page was re-verified --- not the #2246 counterfactual above it, not the #2334 literals, not the Vale or divergence bullets below it."
