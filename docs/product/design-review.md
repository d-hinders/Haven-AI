...
- There is no retired-rail notice to review. #2413 deleted `RetiredRailNotice`
  and the six list queries filter to `account_type = 'delegator_hybrid'`, so no
  legacy account reaches a screen; this criterion is retained only so a reviewer
  who meets such a notice in a diff knows it should not be there.
...
- Capture rendered-screen evidence on one of the three triggers in
  [`ship-playbooks/frontend.md` §4](../contributing/ship-playbooks/frontend.md#4-verification)
  — a **new route**, a **changed shared primitive**, or a diff that **changes what a
  screen shows** (#2636): `npm run screenshot -w packages/frontend -- <routes>`
  (desktop + mobile PNGs; attach to the PR or reference them in Browser
  Verification). A logic-only change to an existing route takes the headless
  equivalent instead — but check whether the route has a committed pixel baseline
  before you rely on that, because on a route without one nothing else is comparing
  it. `/design-system` itself is pixel-guarded by the blocking
...
- Run `haven-reviewer` on **every** pull request — not only user-facing, money,
  ...
  2026-08-21 owner decision; `CLAUDE.md` § *How shipping is governed* is canonical since #2639.
