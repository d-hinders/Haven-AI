...
verified:
  - "#2764: front matter only — SIX `covers:` entries, not the two the issue names. § *Local hint marker* makes a checkable claim about each and no glob reached any of them, measured before and after: `WalletButton.tsx` (\"one call site so far\" — if a second appears or that one goes, the sentence is false), `agent-panel/AgentCard.tsx` (restates the `--v2-warning` scoping clause verbatim in a rationale comment, `:236-245`), `AccountSignersCard.tsx` and `DelegationSendModal.tsx` (\"the #1097 hints are deliberately unmarked\" — adding the marker there falsifies it), and `connect-agent/{ConnectionVerificationFooter,WaitingForConnector}.tsx` (\"three more call sites at a lighter weight, none of them documented\" — a count and a negative claim). Fixing two of six would have left the same hole open for four files the same subsection asserts about. Proven: `--changed=WalletButton.tsx` now lists this doc in the CODE-to-doc direction where it listed nothing before. The optional half of the issue is deliberately NOT taken and is reported instead — the `/design-system` swatch says `--v2-warning` covers \"paused states\" and the issue reads that as contradicting `AgentCard.tsx`; measured, `AgentCard` paints paused with `--v2-warning` at `:94` (bot tile) and `:146` (status pill) and its own comment says so (\"the card has not lost its amber\"). Its \"paused is neither 402/pending-review nor environment identity\" is about the severity MARKER on a paragraph, not the pill. So the swatch is accurate and this document's enumeration is the incomplete one — a body change with its own review surface, left for the issue's author. Scope: the `covers:` list and this note. No prose re-verified."
