...
- **`href` turns it into a link.** With `href`, `Button` renders a `next/link` `<a>` (and
  ...
  differently again. All six `trailingIcon` call sites are the `href` form.
- **`trailingIcon`** renders a 14px lucide `ArrowRight` after the label, at the base
  `gap-1.5`. It is available on **every** variant, not just primary. **It does not animate.**
...
Ghost — **variant**, `variant="ghost"`. By a wide margin the most-used variant:
...
- 14 / 16 / 20 px (matches v1 rule); size via className (`h-4 w-4`) or the wrapper's numeric `size` prop. **Known gap: "exactly" is currently false, and the gap is measured rather than suspected** — roughly a third of sized `<Icon>` call sites are off this scale, including a large 12px cluster and a handful of arbitrary values (`h-[13px]`, `h-[17px]`, `h-[22px]`, …). The counts and the script that produces them live in [#1858](https://github.com/d-hinders/Haven-AI/issues/1858), deliberately, so the figure can be re-derived and corrected rather than cited from here after it goes stale — the same reason the arrow rule below ships a command instead of a number. Treat the three sizes above as the intent, not as a description of what is built, until #1858 closes. Unlike `stroke-width` and `aria-hidden`, this bullet has no mechanical enforcement in `Icon.tsx`, which is why it drifted while its neighbours held.
