...
- **Exactly one gated file may render a raw arrow:** `components/haven/TransactionMovement.tsx`, the `From <a> → To <b>` movement line. The glyph joins two operands the way a colon would — `aria-hidden`, on the text baseline at the run's own size and weight, with the words `From` and `To` carrying the direction. A lucide glyph would sit at a fixed pixel size beside text it must match, and since #1774 it is nested *inside* the `From` half to stop it wrapping alone, so there is no separable icon slot to fill. **Every other raw arrow in a gated surface is a defect**, whatever it is called.
  ...
  **What it returns today: 15 lines** — one allowlisted (`TransactionMovement.tsx:55`), four *continuation* lines of multi-line `{/* … */}` JSX comments that a line-based search cannot distinguish from markup (`TransactionMovement`, `SetupStates`, `WaitingForConnector` ×2), and **ten defects across seven files**, tracked in [#1857](https://github.com/d-hinders/Haven-AI/issues/1857):

  | File | Sites | What |
  |---|---|---|
  | `components/haven/Address.tsx` | 1 | `↗` external-link affordance in the **shared address primitive** — highest value, since `TransactionActivityRow` and `SendModal` already do this correctly with lucide `ExternalLink` |
  | `app/(authenticated)/design-system/page.tsx` | 2 | `title→body` prose in the Spacing blurb, and a caption advertising `Address`'s `↗` — the showcase currently documents the defect as the pattern |
  | `components/AddFundsModal.tsx` | 2 | `→` inside `<Button>`, which already has `trailingIcon` |
  | `app/(authenticated)/accounts/[safeId]/AccountDetailClient.tsx` | 2 | `View all &rarr;` links |
  | `app/(authenticated)/accounts/AccountsOverviewClient.tsx` | 1 | `Open &rarr;` card CTA, animated on `group-hover` |
  | `app/(authenticated)/settings/SettingsClient.tsx` | 1 | `→` on the exit-path link |
  | `app/(authenticated)/custody/page.tsx` | 1 | `↗` on the Safe{Wallet} link |
