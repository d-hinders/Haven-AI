...
**Fitting the track does not fit the header — what a narrow card drops is a product decision ([#2325](https://github.com/d-hinders/Haven-AI/issues/2325)).** The grid-track rule above stopped `/agents` cards growing past their column; the space the card then had to divide exposed the next defect in the same header row. At 390 the card's header row is 300px, and it splits: bot tile 36 + gap 12 + the name/account/MCP block (`min-w-0 flex-1`) 117 + gap 12 + the `ml-auto shrink-0` "Last activity" stamp **123**. The stamp takes 41% of the row and cannot yield, so the MCP chip inside the block gets **34px** — which ellipsises even a short slug: `haven-research` measures **113px** and renders as `haven…` on every card, the opposite of what #1878 added the chip for.
