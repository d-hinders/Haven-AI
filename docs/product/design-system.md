...
`Set active` (72.6×24 painted) and its `Set as default` star (26×26) both take the
mechanism — vertical-only for the labelled one, `h-11 w-11` in both axes for the square,
per the two rules above. Nothing new there. What is new is the defect the measurement
found *underneath* the undersized target, which the issue had diagnosed the other way
round:
...
- **Two adjacent borrowers make the clearance rule bite in-component, not just against a
  ...
  `gap-2.5` leaves 1px. The § *Tap targets* "at least 8px between stacked controls" line
