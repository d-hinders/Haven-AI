...
**Fitting the track does not fit the header — what a narrow card drops is a product decision ([#2325](https://github.com/d-hinders/Haven-AI/issues/2325)).** The grid-track rule above stopped `/agents` cards growing past their column; the space the card then had to divide exposed the next defect in the same header row. At 390 the card's content box is 300px, and the header splits it: bot tile 36 + gap 12 + the name/account/MCP block (`min-w-0 flex-1`) 121.2 + gap 12 + the `ml-auto shrink-0` "Last activity" stamp **118.8**. The stamp takes 40% of the row and cannot yield, so the MCP chip inside the block gets **62.5px** — which ellipsises even a short slug: `haven-research` measures **113px** and renders as `haven…` on every card, the opposite of what #1878 added the chip for.
...
**The 12 px row cites buttons that do not meet the 44 px tap target, and that is a debt it inherits rather than endorses.** the shared `ui/CopyButton.tsx:63`/`:65` (a bare `h-5 w-5`, and so every `Address` and `McpServerName` that renders it) hosts this rung without the invisible `::after` hit area (the hand-rolled ones are gone: `SendModal.tsx:627` deleted by [#1989](https://github.com/d-hinders/Haven-AI/issues/1989), `agent-panel/UnmanagedDelegateCard.tsx`'s replaced with `Address` by [#1980](https://github.com/d-hinders/Haven-AI/issues/1980) — which routes it back through the same `CopyButton` debt rather than retiring it) § Buttons requires — that machinery lives on the `Button` primitive and its one documented borrower. Nothing here changes those boxes. Naming it so the table is not read as blessing sub-44 px targets: an icon-only button owes its own tap-target treatment whatever size glyph it holds.
