...
**Reveal collapsed columns in more than one stage, and do not assume viewport width buys content width.** Two things step at `lg` and both take width: the sidebar goes `fixed` to `lg:static` at `w-[240px]` plus a 1px `border-r`, and the authenticated layout's `main` steps `p-6` to `lg:p-8` (16px more across the pair). 257px is handed back in a single breakpoint, so the content available to a table is a **sawtooth** in viewport width, not a ramp. Attributing it to the sidebar alone is wrong and invites a reader who checks `Sidebar.tsx`, finds 240px, and discards the whole claim. Measured on `/design-system`: the transaction table has 717.9px of content at **768px** and again at **1024px**, and 973px at both 1023px and 1279px. Revealing every column at `md` therefore starved the Activity measure twice — 71.1px across five lines at 768px, and identically at 1024px ([#1827](https://github.com/d-hinders/Haven-AI/issues/1827)). Two consequences. First, a fix keyed to "the `md` breakpoint" is keyed to the wrong variable and will leave `lg` broken. Second, the step that restores a column belongs at the first viewport that clears the target width on **both** teeth — `xl` for a seven-column table, not `lg`. Pinning the `md`+ columns instead is the tempting alternative and it is wrong: it was measured during #1774 and grew desktop rows 85px → 133px. Prefer moving low-priority content rather than dropping it (this table keeps the date under the Amount until `xl`, the same place the below-`md` layout puts it).

**`TransactionsTable.tsx` is not affected by this rule and should not be changed to match it.** It is also a seven-column table and it still reveals at `md`, correctly: it escapes measure starvation a different way, by **truncating its title to one line instead of wrapping it**. Measured, it renders one line at every width in the band — 89.6px at 768px, 225.1px at 1280px — so the defect above simply does not arise there. The rule is about tables whose flexible cell *wraps*. Restructuring `TransactionsTable` for consistency with a showcase would be a change against a defect measurement says is absent, which is the trap § *Local hint marker* already warns about.

Note what can and cannot see this class of defect: the visual-regression gate renders `/design-system` at 1280px and 390px only (`scripts/evidence-viewports.mjs`), so **every width in the band above is invisible to it**, before and after. Geometry assertions are the guard — see `e2e/transaction-title-measure.spec.ts`, which asserts the measure floor and the row-height ceiling *together*, because either alone is satisfiable by a change that destroys the other.
...
**The 12 px row cites buttons that do not meet the 44 px tap target, and that is a debt it inherits rather than endorses.** the shared `ui/CopyButton.tsx:63`/`:65` (a bare `h-5 w-5`, and so every `Address` and `McpServerName` that renders it) and the unsized one in `agent-panel/UnmanagedDelegateCard.tsx:72` (`SendModal.tsx:627` was the other until [#1989](https://github.com/d-hinders/Haven-AI/issues/1989) deleted it) all host this rung and none carry the invisible `::after` hit area § Buttons requires — that machinery lives on the `Button` primitive and its one documented borrower. Nothing here changes those boxes. Naming it so the table is not read as blessing sub-44 px targets: an icon-only button owes its own tap-target treatment whatever size glyph it holds.
