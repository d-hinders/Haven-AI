...
- **An ancestor `inert` + `aria-hidden="true"` wrapper is what removes the copy
  ...
  does not.** Measured in real Chromium on
  [#1982](https://github.com/d-hinders/Haven-AI/issues/1982): with the showcase
  popovers forced back to a bare `role="dialog"` inside an intact wrapper, the
  document held three `role="dialog"` nodes and Playwright's
  accessibility-tree-mediated `getByRole('dialog')` still resolved to exactly
  one. A role swap moves raw DOM queries and document-wide role counts and
  nothing else — a real job, since `e2e/modal-scroll-cue.spec.ts` asks
  document-wide with `document.querySelector('[role="dialog"]')`, but a
  different one. Keep both; neither is redundant because the other exists, and
  dropping the wrapper on the strength of the role would expose a labelled
  control group whose handlers do nothing.
