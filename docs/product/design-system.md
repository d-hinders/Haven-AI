...
> **⚠️ Four of the five tokens above currently paint NOTHING, and this section
> described them as if they did.** Measured on `dev` at
> [#1893](https://github.com/d-hinders/Haven-AI/issues/1893) by reading the
> served stylesheet in a real browser: `.shadow-[var(--v2-shadow-card)]` and its
> siblings compile to
> `--tw-shadow-color: var(…); --tw-shadow: var(--tw-shadow-colored);`, and
> nothing ever sets `--tw-shadow-colored`, so the computed `box-shadow` is
> `none`. A bare `var()` inside `shadow-[…]` is ambiguous and Tailwind's
> arbitrary-value type inference reads it as a shadow **colour**. So every
> `Card`, `ui/Modal`'s own panel, and every popover render flat today.
>
> This is the same dead-arbitrary-value class as § "Opacity on a token colour"
> and the #1708 focus ring — a spelling that lints clean, reads correctly, and
> emits nothing. It is **not fixed here**: restoring four shadows app-wide moves
> pixels on nearly every surface and needs its own baselines and design pass.
> Filed as a follow-up. Recorded now so the tokens are not read as shipped
> behaviour, and so nobody re-derives the diagnosis a third time. Follow-up:
> [#1945](https://github.com/d-hinders/Haven-AI/issues/1945).
>
> The working idiom, when you need a token-valued shadow, is a named class in
> `globals.css` — `.v2-modal-backdrop` and `.v2-scroll-edge-cue` both resolve.
...
3. **No transition, deliberately.** § 4 bans entrance animations on first paint,
   and the cue's first honest reading lands in an effect *after* the first
   commit — so a fade-in would animate on first paint in every overflowing
   dialog in the app. `docs/product/design-review.md` additionally requires any
   animation to be gated behind `prefers-reduced-motion: no-preference`. At 6px
   the appear/disappear is a hairline, not a flash.
...
and `ReplaceSigningKeyModal` do this. `DelegationSendModal`,
`contacts/page.tsx` and `ComingSoonModal` instead render their action rows as
the last children *inside* the scroll body, which means their buttons scroll
with the content and sit under the cue's 6px edge when the body overflows. They
remain clickable, and at 6px the visual cost is slight — but it is an
inconsistency in the primitive's contract rather than a preference. Tracked as
[#1946](https://github.com/d-hinders/Haven-AI/issues/1946); do not add new
dialogs in that shape.
