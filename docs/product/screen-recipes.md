...
- Accounting connections are a Settings section (owner decision 2026-09-11, #2868): one `Row` per provider with a status chip and the one action that resolves the state; the feed settings open inline under the row (`Card.Section`-style block, never a nested filled card); Disconnect confirms and says history in Haven stays; copy follows the accounting guardrail — a payment *appears in the ledger with payment evidence attached; the accountant books it*.
