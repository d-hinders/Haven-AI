...
key IS on that allow-list, so the sentence above about printing a freshly
rotated key holds for the `owner_cli` token too — the approval screen names it
in what the session can do.
