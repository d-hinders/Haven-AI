...
- **skip-settle chain gate (PR #1277)** — the demo merchant's verify-without-settle QA hook (#603) is now code-gated to Base Sepolia like the erc7710 flag: startup refuses with `MERCHANT_SKIP_SETTLE_PRODUCT` set on any other chain. Before this, one copy-pasted env var on a mainnet instance made every listed product deliverable against a merely well-formed EIP-3009 authorization from an empty wallet — the only balance check lives inside the settlement call the hook skips. Adds a refusal, removes nothing; merchant-side only, no Haven authority or custody surface involved. Perimeter tightened.
