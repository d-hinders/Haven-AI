...
- User treasury funds are held in the user's smart account — a Hybrid DeleGator on the live delegation rail; retired-rail Safes remain user-owned, and their rows readable to a direct database query, though since #2413 no account, agent or dashboard surface displays them. An agent-held delegate EOA may also hold a transient balance used for an EIP-3009 x402 merchant payment; Haven controls neither account.
...
User funds
  ...
  -> retired-rail Safes remain user-owned; rows readable to a direct DB query,
     displayed by no account/agent/dashboard surface since #2413 (the transactions
     picklists are the named exception -- see the retirement paragraph above)
...
- **Directory globs beat file lists** where a whole layer is the boundary.
  ...
  entry was correct while the legacy rail was the only rail (as of this
  rewrite, the delegation rail is the only live one — see the roster pinned by
  [`packages/backend/src/__tests__/execution-rail-live-census-pin.test.ts`](../../packages/backend/src/__tests__/execution-rail-live-census-pin.test.ts),
  #2680) and silently stopped
...
- *Coverage by some other doc is a different obligation, not a substitute for a
  perimeter analysis.* `publish.yml`'s three contract docs
  (`branch-and-release-flow.md`, `mcp-runtime-compatibility.md`, and since
  #2425 `package-dev-channel.md`) describe the branch model, the runtime pins
  and the dev snapshot channel; none of them asks the authority question. It
  holds the publish trigger, the ref it builds from, the dist-tag choice and the
  registry credential paths — since #2647 two of them, an OIDC credential for
  publishing and a `main`-only environment token for moving `latest` — so it is
  covered on the same footing.
...
- **#1346** — Connect completion handoff made runtime-aware (PR #1354): the connector's final output orders the user's next steps per runtime (approve rules in Haven first, activate the runtime second, verify with the READ-ONLY `haven_get_agent`/`haven_get_allowances` — the verification must not sign, fund, or create a payment) and activation guidance comes from the Connect runtime registry. Orientation prose and ordering only: no credential handling changed, no new capability, the no-secret output boundaries (no keys, no credential paths, abbreviated delegate address) hold. Perimeter unchanged. *(Entry added retroactively in the 2026-08-12 docs-staleness sweep — the shipping PR bumped only the header.)*
- **#1352** — the agent-runtime Node floor (`HAVEN_MINIMUM_NODE_VERSION`, single source per #1161, with `engines.node` in sdk/connect/signer/mcp guard-test-pinned to it) was lowered from `24.0.0` to `22.0.0`: nothing in the runtime uses an API newer than `AbortSignal.any` (Node 20.3), Node 20 is past end-of-life, and 22 is the oldest still-supported LTS. The #1161 enforcement points are UNCHANGED — connect still refuses setup and the signer still refuses startup below the floor, fail-closed before any key handling — only the threshold moved, and the derivation chain (manifest `minimumNodeVersion` from the SDK constant, refusal hints from the same constant) means no second literal was introduced. `.nvmrc` stays LTS 24 as the repo/CI pin, which by rule sits at or above the user-facing floor, never below. No key material, no route, no refusal removed, no authority change: a wider set of user machines may now run the same perimeter. Perimeter unchanged.
- **#1347** — structured Connect completion outcome (PR #1356): `--json` emits one parseable, versioned object on stdout (schema_version 1; outcome complete/action_required/failed; runtime, configuration/probe state, activation, next action, approval expiry, read-only verification guidance) while progress prose stays on stderr; library callers read the same object at `runConnect(...).outcome`. Redacted by construction: no API/private keys, no credential contents or full credential paths, no full delegate address serialized. Reporting shape only — no credential authority, route, or approval-boundary change. Perimeter unchanged. *(Entry added retroactively in the 2026-08-12 docs-staleness sweep — the shipping PR bumped only the header.)*
- **#1355** — true payment_id-only signing: the merchant's 402 `PaymentRequired` is now persisted on the intent row's `machine_metadata` at authorize time (the EXACT #1307 pattern — same JSONB blob, no migration, bounded at 64KB with a loud 400 above it) and re-served by the EXISTING read-only `GET /x402/:id/sign-context`, so `haven_sign_x402` accepts `{ payment_id }` alone and the agent relays no payment blob at all. Authority analysis: the stored blob is delivery/reporting material, not payment authority — whichever copy the signer uses (context-fetched preferred, caller-supplied fallback for pre-#1355 backends) passes through the UNCHANGED `assertX402MatchesExpected` verification against the Haven-signed expected context (resource, merchant recipient, amount, asset, network), and that guard is mutation-proven: a stored blob paying a different merchant than the signed context refuses (test fails when the guard would be bypassed). No independent trust leg was removed — the agent-relayed copy also originated from Haven's prepare/pay response, so folding it into the Haven-authenticated context fetch changes provenance, not trust. The signer's single network capability remains the same authenticated READ (#1263); nothing new is signable, no refusal was removed, no route was added. Perimeter unchanged.
- **#1350** — catalog discovery gained case-insensitive `category` matching and an additive read-only `search` filter across product `name`, `description`, and `category`, exposed consistently through `GET /catalog`, `HavenClient.discoverTools`, and both MCP `haven_discover_tools` surfaces. This widens only how an agent FINDS curated metadata; it does not create a quote, signature, approval, or payment request, and the catalog's displayed price remains explicitly indicative while the live merchant 402 stays authoritative for recipient, asset, network, amount, and settlement options. API-key auth remains identity-only, chain/rail/status scoping still constrains visibility, and empty or ambiguous searches only change which rows are returned. Perimeter unchanged.
- **#1348** — guided-purchase round-trip reduction, orchestration only: the preflight's independent Haven reads (agent, allowances) now overlap the merchant probe instead of following it, `getAgent` coalesces CONCURRENT calls into one HTTP GET (in-flight dedupe, never a cache — sequential calls remain fresh reads), and `createX402Intent` accepts the caller's already-fetched `delegateAddress` instead of re-fetching. Staleness honesty (#1358 review): the backend compares `payTo` against the CURRENT delegate address to derive the funding shape, so an address made stale by a delegate rotation inside the widened window (one tool call — the merchant-quote duration, previously sub-millisecond; never externally suppliable) is a clean 403 for pinned-budget agents but would route an open-budget delegation agent to the settlement shape with the stale address — pre-existing in kind, widened in window, tracked for server-truth hardening in #1360. Every refusal, its ordering guarantee, and its pre-funding position are unchanged and remain test-pinned: the live-quote cap check still precedes intent creation, the agent-lookup failure is still a hard refusal before any intent exists, an allowance-read failure still degrades to a warning, and the deterministic error precedence (quote error wins) is now itself asserted. No key material, no route, no new read scope — the same reads, fewer times, sooner. Perimeter unchanged.
- **#1349** — hosted `haven_settle_mcp_tool` now adds `agent_summary.purchase_summary` as a compact reporting contract after the existing funding and paid-merchant completion succeed. Its settlement status, money/asset/network, merchant address/resource endpoint, funding hash, and allowance are derived from Haven's established flow and payment state; merchant content can contribute optional product/invoice display metadata, and `settlement_tx_hash` is only an optional merchant PAYMENT-RESPONSE receipt reference. Absent values are explicit as `null`. The raw merchant `result` is retained separately as evidence and cannot make a payment appear settled. No signed intent, payment header, signer, backend authority, route, or on-chain policy changes; this is read-only response shaping and does not make Haven a merchant settlement provider. Perimeter unchanged.
- **#1351** — the per-purchase spending cap gained an unambiguous human-unit spelling: `max_amount_human` (whole tokens — `"1"` is 1 USDC) alongside the UNCHANGED atomic `max_amount`, on `haven_pay_mcp_tool`, `haven_pay_x402_quote` and `haven_prepare_catalog_purchase`. Authority analysis: this is an input-ergonomics fix to a guard that only ever NARROWS spend, and the cap remains an ADDITIONAL per-purchase user-intent ceiling on top of the on-chain budget — never a replacement, never a grant. The conversion is bound to the LIVE quote (`X402Quote.decimals`, resolved from the SAME address→token binding that produces `token`), so a caller cannot widen the cap by naming a different token, and the on-chain delegation budget still refuses over-budget purchases with the cap satisfied (test-pinned). Three refusals were ADDED, all pre-funding and all fail-closed: `AMBIGUOUS_MAX_AMOUNT` when both spellings arrive (before ANY network call, even when the two agree — honouring one silently would teach the pattern), `INVALID_INPUT` when the guided path gets neither, and `MAX_AMOUNT_UNCONVERTIBLE` when the human cap cannot be converted against this quote — unknown asset decimals (reachable: the option selector checks network and asset against separate sets, so Base-Sepolia USDC on mainnet Base is selectable but resolves to no known token, and `decimals` is then `null`) or more fraction digits than the asset holds, where truncating would silently move the user's cap. A fourth, found in review: `haven_pay_x402_quote` previously SKIPPED the cap when no payment option was settleable, leaving the agent believing a purchase was capped when nothing had been compared — either spelling now refuses there instead (blast radius characterized: the uncapped call is unchanged, and that case already failed one step later inside `createX402Intent`, which is why the old skip could never actually fund an unchecked purchase). No refusal was removed or relaxed, no route added, no key material touched, and the existing atomic contract is byte-for-byte unchanged for existing callers (characterization-tested: `max_amount: "1"` still means 0.000001 USDC). The guards are mutation-proven — treating the human cap as atomic fails 9 tests, severing the ambiguity check fails 3, severing the precision check fails 1. Perimeter unchanged.
- **#1362** — Fortnox reporting-feed visibility: the dashboard now shows the pushed supplier-invoice number per synced payment, and a new read-only `GET /accounting/reporting/verify/:paymentId` performs the feed's FIRST read-back against Fortnox — does the invoice we pushed still exist (registered), and has a human booked it (voucher assigned)? Perimeter analysis: strictly read-only (test-pinned: a full verification makes no POST and writes nothing), user-scoped through the same entitlement gate as the sync action, and the non-asserting principle (#491) is untouched — Haven still creates only unattested supplier invoices and cannot book, cancel, or modify them; verification REPORTS the accountant's action, never performs it. The cross-check that the invoice carries our `HAVEN-<paymentId>` external number is mutation-proven (a number collision after a Fortnox company switch must read as not-found, never as falsely registered). A new operations runbook (docs/operations/fortnox-reporting-feed.md) documents the purchase→Fortnox flow, the verification walkthrough, and the failure taxonomy. No payment authority, no custody, no key material — observability over an existing reporting write path. Perimeter unchanged.
- **#1360** — server-truth hardening of the funding-shape decision, closing the #1358-review gap: the SDK's two EIP-3009-shape writers (`createX402Intent` and the local-key `authorizeX402`) now ALWAYS send `settlementScheme: 'eip3009'`, so a `payTo` made stale by a delegate rotation mid-flow hits the EXISTING `validateDelegationSchemeShape` refusal (400, before any funding intent, selector never consulted — test-pinned) instead of silently reading as a merchant address and routing an open-budget agent to the erc7710 settlement branch. No new validation was added and none was removed — the fix is that the intent's meaning is now declared rather than inferred from an address comparison. The pre-#1360 shape-only selection is characterization-tested for old SDKs (a stale `payTo` without the declaration still reaches the erc7710 selector — documented, not hidden), and legacy-rail backends accept and ignore the declaration (compatibility-pinned). No authority, route, or verification change; a misroute class became a refusal. Perimeter unchanged.
- **#1328** — the legacy internal `mpp_demo` merchant flow is RETIRED, the #834 session-rail pattern applied to a flow instead of a whole rail: `POST /demo/mpp/*` (`routes/demo-mpp.ts`, its `MPP_DEMO_RECIPIENT_ADDRESS` config, and its route-specific tests) is deleted outright; `POST /machine-payments/authorize` now refuses UNCONDITIONALLY with HTTP 410 before the body is inspected or `authorizeMachinePayment` is called — fail-closed, mutation-proven (severing the refusal made three characterization tests fail); a forward migration (059) DELISTS the seeded "Haven MPP demo resource" catalog row (status vocabulary, not a delete — `GET /catalog` and the periodic refresh probe already exclude `status = 'delisted'`, proven on real Postgres) so discovery stops returning it and the probe stops re-verifying a route that no longer exists. The SDK's demo challenge/quote/resume client surface (`mpp.ts`; `quoteMpp`/`payMppChallenge`/`resumeAuthorizedMpp`/`resumeMppPayment`/the `MACHINE-PAYMENT-CHALLENGE` branch in `haven.fetch()`) and both MCP tool surfaces' `haven_quote_mpp`/`haven_pay_mpp_challenge`/`haven_resume_mpp_payment` tools are removed the same way — MACHINE-PAYMENT-CHALLENGE was never produced by anything besides the now-deleted demo route, confirmed by search before deleting. Narrows the surface; adds no authority, no key material, no new signing path. What is explicitly PRESERVED, unchanged: the generic `/machine-payments/*` reads (identity, allowances, status, receipts, evidence, reconciliation, sweep) and existing `mpp_demo` payment/receipt/evidence/status rows, which stay readable (mutation-proven — severing the evidence-eligibility gate for `mpp_demo` made seven historical-read tests fail); the rail-agnostic `authorizeMachinePayment` orchestration in `modules/mpp/authorize.ts` (kept, unreferenced by any route today, for a possible future non-demo MPP rail — out of this issue's scope per its own file-ownership note); and the `mpp_crypto`/`stripe_deposit`/`spt` type vocabulary everywhere it appears (`MachinePaymentRail`, `AgentPaymentRail`), touched nowhere — the issue explicitly reserves any change there for a separate, reviewed follow-up. Review hardening (#1339 finding 1): historical mpp_demo records are now non-EXECUTABLE as well as non-creatable — the generic sign path (`POST /payments/:id/sign`) refuses an mpp_demo intent 410-with-nothing-written before the claim (mutation-proven: removing the rail guard flips the test to a live execution), and the approval surface refuses approve/proposed for mpp_demo rows via a race-free WHERE-clause exclusion (reject remains allowed as cleanup). The retained `POST /machine-payments/authorize` 410 is a DOCUMENTED deliberate exception (compatibility tombstone — a 410 tells old clients the flow is permanently gone where a 404 invites retries), recorded in the OpenAPI description. Perimeter narrowed, not widened.
