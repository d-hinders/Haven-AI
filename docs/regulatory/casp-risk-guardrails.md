...
- **#1255** — x402 delegation-rail signer handoff hardening. Hosted MCP now returns `sign_data` as the canonical object for the local signer, self-checks that `sign_data`, compatibility aliases, `x402.expected`, and the committed typed-data digest agree before returning a quote, and the signer accepts `sign_data` directly while refusing conflicting mixed fields. Backend/SDK/signer digest derivation uses one SDK helper, and direct `viem` pins are exact-aligned so separately installed runtimes hash EIP-712 payloads consistently. No key moved server-side, no validation weakened, no new authority or discretion added: this narrows the handoff and keeps the local agent-held signature as the spend authority. Perimeter unchanged.
