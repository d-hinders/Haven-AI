...
| [Bookkeeping-ready export (Fortnox / SIE)](bookkeeping-ready-export.md) | **Superseded in direction by the non-asserting feed (#491).** Original "audit-ready / assert finished vouchers + VAT" design — book-time SEK capture, canonical accounting record, rail-agnostic exporter. Foundation reused; the asserting framing is not. |
| [Haven's position on AP2](ap2-position.md) | Should Haven support Google's Agent Payments Protocol? Mandate-vs-delegation mapping, why AP2 is an evidence layer rather than a control layer on a self-custody rail, and the recorded **monitor-only** decision with its revisit triggers. ([#2030](https://github.com/d-hinders/Haven-AI/issues/2030)) |
