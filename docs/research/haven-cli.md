...
> flags plus `env`) and five flag NAMES coincide — `--api`, `--help`,
> `--json`, `--name`, `--version`, and the last two mean different things on
> the two sides — so sharing one parser would be a rewrite rather than an
> extraction; `redact.ts` exists only in connect and the CLI redacts nothing;
...
> (`packages/cli/src/session.ts:48-50`). Connect writes the same two syscalls
> in five places but through `restrictPermissions` (retry-and-warn) and
> `writeOwnerOnlyJson` (atomic `wx`, `dropUndefined`), neither of which the
> CLI replicates, so what is genuinely common is two calls with two modes.
> That does not pay for a package of its own, since `@haven_ai/cli` is published with ZERO runtime
