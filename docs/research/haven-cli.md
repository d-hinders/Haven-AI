...
> flags plus `env`) and only three flag NAMES coincide, so sharing one would
> be a rewrite rather than an extraction; `redact.ts` exists only in connect
> and the CLI redacts nothing; the six-code exit table exists only in the CLI
> and connect returns a bare 0/1. Exactly one had real overlap — the ~20-line
> 0600 write (`mkdir` 0700, `writeFile` 0600) — which does not pay for a
> package of its own, since `@haven_ai/cli` is published with ZERO runtime
