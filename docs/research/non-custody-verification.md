...
since #2413 no Haven surface displays them: the account and agent list queries filter
...
no read surface after it — the owner-access read that once gated a
Safe interface link (`useRetiredRailOwnerAccess`) went with it, so Haven makes no
self-serve Safe promise to any legacy account holder, owner-identified or not. Any
