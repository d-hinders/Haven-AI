...
since #2413 no account, agent or dashboard surface displays them: the account and agent list queries filter
to `account_type = 'delegator_hybrid'`, so balances, activity, transaction history and
agent identity no longer render, and `RetiredRailNotice` is deleted along with every
`account_type` branch behind it. The legacy rail had no agent create, budget,
payment-credential, or Haven-mediated authority-revocation action before that, and has
no Haven-RENDERED read surface after it — the hook that gated the
