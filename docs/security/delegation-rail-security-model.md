...
EOA when the set names an owner → any passkey — so a mixed account keeps
...
branch now answers `ready` for a non-empty set for the same reason;
`passkey_on_other_device` remains the legacy-Safe answer, where the block is
real because the stored signer metadata a Safe passkey needs is genuinely
