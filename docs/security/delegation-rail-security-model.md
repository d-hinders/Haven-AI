...
key works from the backup device (`lib/delegationPasskeySigner.ts` — the one
place the credential is chosen; `pickSigningPath` in `hooks/useDelegationBudget.ts`
