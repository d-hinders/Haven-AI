...
stale on the next pull request — and asserts there is no third state:
allow-listed, or refusing. It also checks the reverse direction, and caught
three entries naming routes that do not exist on its first run; an entry for a
missing route grants nothing while reading like coverage.
