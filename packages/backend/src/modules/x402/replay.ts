  ...
  if (existing.status === 'confirmed' && existing.tx_hash) {
    return {
      ...
      body: {
        ...
        account_address: existing.account_address,
...
export async function rebuildDelegationSignContext(
  ...
  const sign_data = isErc7710State
    ...
    : {
        ...
        components: {
          // #2914: replaced the deprecated `safe` key. NOT `account`, which
          // is a different address on this shape — see the sibling comment in
          // `delegation-authorize.ts`.
