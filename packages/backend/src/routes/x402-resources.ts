...
export default async function x402ResourceRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post<{ Params: { id: string }; Body: VerifyBody }>(
    ...
    async (request, reply) => {
      ...
      const verification = await verifyAllowanceTransferTx(
        resource.chain_id,
        tx_hash,
...
// On-chain AllowanceModule transfer verification moved to
// infra/chain/allowance-transfer-verifier.ts (#994) — decoding
// `executeAllowanceTransfer` calldata needs `ethers.Interface`, which
// routes/** may no longer import directly.
