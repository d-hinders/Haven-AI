...
describe('haven guide text (#2525)', () => {
  it('is byte-for-byte the canonical SDK runbook', async () => {
    ...
    // Both figures, because they differ and each gets quoted somewhere: 10,059
    // UTF-8 bytes, 9,978 UTF-16 code units. The em-dashes are the gap — the
    ...
    // (`<host>/dashboard`) instead of "the dashboard", and by #2618, whose
    // step-1 names the non-blocking login sequence: `--no-wait` under --json,
    // then `haven login --poll <device_code>` — the flow an agent can run
    // without holding its turn open for ten minutes.
    //
    ...
    expect(Buffer.byteLength(HAVEN_AGENT_RUNBOOK_MD, 'utf8')).toBe(10059)
    expect(HAVEN_AGENT_RUNBOOK_MD.length).toBe(9978)
  })
