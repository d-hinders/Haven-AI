...
describe('haven guide text (#2525)', () => {
  it('is byte-for-byte the canonical SDK runbook', async () => {
    ...
    // Both figures, because they differ and each gets quoted somewhere: 8,880
    // UTF-8 bytes, 8,807 UTF-16 code units. The em-dashes are the gap — the
    ...
    // `haven wallets funding`, and by #2591.
    //
    ...
    // `--api` flag: the CLI defaults to localhost, no agent-facing surface
    // said so, and an agent following the prose path alone was talking to
    // nothing.
    expect(Buffer.byteLength(HAVEN_AGENT_RUNBOOK_MD, 'utf8')).toBe(8880)
    expect(HAVEN_AGENT_RUNBOOK_MD.length).toBe(8807)
  })
