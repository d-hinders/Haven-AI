...
describe('haven guide text (#2525)', () => {
  it('is byte-for-byte the canonical SDK runbook', async () => {
    ...
    // Both figures, because they differ and each gets quoted somewhere: 9,102
    // UTF-8 bytes, 9,027 UTF-16 code units. The em-dashes are the gap — the
    ...
    expect(Buffer.byteLength(HAVEN_AGENT_RUNBOOK_MD, 'utf8')).toBe(9102)
    expect(HAVEN_AGENT_RUNBOOK_MD.length).toBe(9027)
  })
