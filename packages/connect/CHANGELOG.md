...
- **The tombstone ledger follows the credential root (#3251).** `--tombstone`, `--unwire` and `--replace` previously wrote every retirement into the ambient `~/.haven/tombstones` whatever root held the retired directory; the ledger now sits beside that root. The default root still maps to `~/.haven/tombstones`, so an install that never passed `--credentials-dir` sees no change. `--doctor` reads the ledger beside the root it scans.
