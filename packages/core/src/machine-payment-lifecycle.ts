/**
 * Machine-payment lifecycle domain (#987, epic #980 M1) — which rails count
 * as machine payments, the flow-status and attention-reason unions, and the
 * pure state derivation. Moved verbatim from the backend so backend and
 * frontend share ONE definition: the backend derives the status it serves,
 * the frontend maps the SAME union to presentation — exhaustively, so a new
 * state added here is a frontend compile error, not a silently missing label.
 */
export const MACHINE_PAYMENT_RAILS: ReadonlySet<string> = new Set(['x402', 'mpp_demo', 'mpp_crypto', 'spt'])
