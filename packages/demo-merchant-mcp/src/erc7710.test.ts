...
import { BaseError, ContractFunctionRevertedError, ExecutionRevertedError, InternalRpcError, LimitExceededRpcError, NonceTooLowError, RpcRequestError, TimeoutError, TransactionExecutionError, TransactionRejectedRpcError, encodeAbiParameters, encodeErrorResult, keccak256, parseAbi, type Address, type Hex } from 'viem'
...
describe('restart survival — the chain remembers what the maps forget (#1515)', () => {
  ...
  describe('a revert at submit is explained, not reported as a merchant fault (#3170)', () => {
    ...
    // Build the errors exactly as viem's `writeContract` throws them: every
    // failure — revert or not — arrives wrapped in a ContractFunctionExecutionError
    // (getContractError), and node-level failures sit inside a
    // TransactionExecutionError first. The classifier must see through the wrapper.
    const CONTRACT = { abi: parseAbi(['function redeemDelegations(bytes[] p, uint8[] m, bytes[] e)', 'error AllowanceExceeded(uint256 spent)']), address: MERCHANT, functionName: 'redeemDelegations' }
    function asWriteContract(inner: BaseError): BaseError {
      return getContractError(inner, { ...CONTRACT, args: [], sender: MERCHANT })
    }
    function asSendTransaction(inner: BaseError): BaseError {
      return asWriteContract(new TransactionExecutionError(inner, { account: { address: MERCHANT, type: 'json-rpc' }, chain: undefined }))
    }
    function rpcError(code: number, message: string, data?: Hex): RpcRequestError {
      return new RpcRequestError({ body: {}, url: 'https://rpc.example', error: { code, message, ...(data ? { data } : {}) } })
    }
    const revertWithData = asWriteContract(rpcError(3, 'execution reverted', encodeErrorResult({ abi: parseAbi(['error Error(string)']), errorName: 'Error', args: ['ERC20TransferAmountEnforcer:allowance-exceeded'] })))
    const revertWithCustomError = asWriteContract(rpcError(3, 'execution reverted', encodeErrorResult({ abi: CONTRACT.abi, errorName: 'AllowanceExceeded', args: [1n] })))
    const revertNoData = asSendTransaction(new ExecutionRevertedError({ cause: rpcError(-32000, 'execution reverted'), message: 'execution reverted' }))
    const nonceTooLow = asSendTransaction(new NonceTooLowError({ cause: rpcError(-32000, 'nonce too low'), nonce: 7 }))
    ...
    it('isContractRevert: only a proven revert is a revert — every other writeContract failure is the merchant\'s fault', () => {
      ...
      expect(isContractRevert(revertWithCustomError)).toBe(true)
      expect(isContractRevert(revertNoData)).toBe(true)
      // The wrapper alone proves nothing: these are merchant-side node faults.
      expect(isContractRevert(nonceTooLow)).toBe(false)
    ...
    it('a nonce fault on the merchant\'s own settlement key stays a merchant FAULT with its reason code — never "a fact about the payer"', async () => {
      const client = submitRevertingClient({ spent: 0n, error: nonceTooLow })
      const { url } = await startServer({ erc7710Client: client, options: ERC7710_OPTIONS })
      const unpaid = await postBuyVpn(url)
      const paymentRequired = await unpaid.json() as PaymentRequired
      const rejected = await postBuyVpn(url, { [PAYMENT_SIGNATURE_HEADER]: erc7710Header(paymentRequired, { permissionContext: realPermissionContext() }) }, 2)
      ...
      expect(rejected.status).toBe(402)
      expect(body.error).toContain('merchant-side fault')
      expect(body.error).not.toContain('fact about the payer')
    ...
    it('a real writeContract revert (decoded data) is REFUSED payer-side, and the next action survives the hosted 500-byte relay window', async () => {
      ...
      expect(body.error).toContain('allowance-exceeded')
      expect(body.reason_code).toBeUndefined()
      // paid-mcp-completion.ts relays JSON.stringify(body).slice(0, 500); `error` is the first key.
    ...
    it('isContractRevert: enforcer strings from a custom client are reverts; gas, RPC and timeouts are not', () => {
