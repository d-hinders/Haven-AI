...
1. **HUMAN — create the account.** Name, email, password, then a passkey (Face ID / Touch ID) or a wallet. Never offer to enter any of it: you must not have their password, and the passkey is bound to their device. With a terminal, `npx @haven_ai/cli login --api <api-url>` afterwards gets you a scoped session for steps 3-4 (every `haven` command needs that flag or `HAVEN_API_URL`: the CLI defaults to localhost, so without it you are talking to nothing) — they approve a code in the browser, you never hold their password. It can set up agents and read the account; it cannot sign, approve a budget, move funds, or rotate any agent's keys.
2. **HUMAN — fund it.** USDC only, no ETH: Haven sponsors the gas. `haven wallets funding` prints the address, the amount **and which chain**; without a CLI session the dashboard shows all three. Read the chain off whichever you used and put it in your message — never assume one: a testnet deployment and production both call themselves Haven.
