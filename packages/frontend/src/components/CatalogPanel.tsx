...
function CatalogCard({
  ...
  return (
    <article
      className={`rounded-xl border border-[var(--v2-border)] bg-white p-4 flex flex-col gap-3 ${
        degraded ? 'opacity-60' : ''
      }`}
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <h3 className="text-sm font-semibold text-[var(--v2-ink)] truncate">{entry.name}</h3>
          <p className="mt-0.5 text-xs text-[var(--v2-ink-3)] line-clamp-2">{entry.description}</p>
        </div>
        <span className="shrink-0 rounded-full bg-[var(--v2-surface-2)] px-2 py-0.5 text-[10px] font-medium uppercase tracking-wide text-[var(--v2-ink-2)]">
          {entry.rail}
        </span>
      </div>

      <div className="flex items-center justify-between text-xs">
        <span className="font-medium text-[var(--v2-ink)] v2-tabular">
          {entry.price_display ?? 'Price on request'}
        </span>
        <span className="text-[var(--v2-ink-3)]">{freshness(entry.verified_at)}</span>
      </div>

      {degraded ? (
        <p className="rounded-lg bg-[var(--v2-warning-soft)] px-3 py-2 text-xs text-[var(--v2-warning)]">
          This service did not answer its last availability check. It may be temporarily down —
          payments could fail until it recovers.
        </p>
      ) : (
        <>
          {budget !== null && (
          ...
          <div className="rounded-lg bg-[var(--v2-surface-2)] px-3 py-2">
            <div className="flex items-center justify-between gap-2">
              ...
              <button
                onClick={copy}
                className="shrink-0 rounded px-2 py-0.5 text-[11px] font-medium text-[var(--v2-brand)] hover:bg-white transition-colors"
