...
function MerchantCell({ agent }: { agent: AnalyticsAgentRow }) {
  ...
  // The label is the API's (contact, else receipt name, else address) — the
  // table never re-resolves it, because a second resolution here is a second
  // place the same merchant can be named two different ways.
  return <span className="text-sm text-[var(--v2-ink)]">{agent.top_merchant.label}</span>
}
    ...
    <div className="px-4 py-3">
      ...
      {open && (
        <dl className="mt-2 space-y-1.5 rounded-md bg-[var(--v2-surface)] p-3 text-xs">
          ...
          <DisclosureLine term="Top merchant" value={agent.top_merchant?.label ?? 'None in this range'} />
          <DisclosureLine
            term="Last payment"
