...
const DEMOS = {
  x402: {
    ...
    timeline: {
      ...
      sign: ['Rules cleared; agent signed the transfer', 'sign_hash 0x8b2f4e93…2c6d4e8f', 'brand'],
      broadcast: ['Settled to the merchant on Base', 'Haven account → merchant · USDC', 'brand'],
  ...
  mpp: {
    ...
    timeline: {
      ...
      rules: ['Rules checked', 'Per-payment limit · network allowlist · allowance', 'brand'],
      sign: ['Rules cleared; agent signed the transfer', 'sign_hash 0x6c1f4e93…2c6d9b3a', 'brand'],
      broadcast: ['Settled to the merchant on Base', 'Haven account → merchant · USDC', 'brand'],
  ...
  return (
    <div
      className={`rounded-[10px] border p-5 transition-all duration-200 ${
        active
          ? 'border-brand/45 bg-[var(--v2-brand-soft)]/55 shadow-[0_12px_32px_-18px_rgba(79,70,229,0.34)]'
          : done
