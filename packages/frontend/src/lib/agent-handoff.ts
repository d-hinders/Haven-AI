...
export function buildHandoff(input: HandoffInput): HandoffArtifacts {
  ...
  const markdown = ([
    ...
    `spend automatically. The SDK surfaces this as a structured`,
    `\`HavenPaymentStateError\` / tool result with \`status: "pending_approval"\`,`,
    `\`payment_id\`, \`phase\`, and \`next_action\`. Tell the user the request is`,
    `waiting in Haven, then call \`get_payment_status\` with that payment id later.`,
    ``,
    `For x402, remember there are two steps: first the user may need to approve`,
