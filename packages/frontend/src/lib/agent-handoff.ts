...
export function buildHandoff(input: HandoffInput): HandoffArtifacts {
  ...
  const markdown = ([
    ...
    ``,
    `For x402, follow the SDK's structured results: retry the original paid API`,
    `request and send the merchant x402 payment header only when Haven reports`,
    ...
    `session, or retry a declined payment in a tight loop.`,
    ``,
