...
export const HOSTED_INSTRUCTIONS = [
  ...
  '',
  'When YOU retry a merchant yourself (the plain-HTTP x402 path), set BOTH',
  'payment header names to the payment_header: PAYMENT-SIGNATURE (x402 v2) and',
  'X-PAYMENT (v1). A strict v2 merchant reads only the first, so sending the',
  'legacy name alone is indistinguishable from sending no header at all — while',
  'on the EIP-3009 bridge the funding leg has already moved the money.',
  '',
