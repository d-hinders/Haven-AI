/**
 * Hosted MCP tool HANDLERS — the #2807 compatibility facade.
 *
 * The tool contracts (names, schemas, input-policy decisions, descriptions,
 * payload types) now live in `tools/contracts.ts`; the registration seam in
 * `tools/registry.ts`; argument parsing in `tools/parsing.ts`. This module
 * re-exports the contract surface unchanged — `index.ts`, `server.ts`, tests
 * and embedders import it exactly as before — and owns the handler behaviour
 * (`createToolHandlers`) plus error normalization. Later capability slices
 * (#2809–#2812) move the handlers out; `HostedToolError` stays here until
 * #2808's shared safety support lands.
 */
...
// #2807: the parsing seam throws through THIS module's HostedToolError so
// `normalizeError` keeps a single instanceof branch. Set at module load; the
// arrow only reads the class when invoked, long after declaration.
