...
| `x402-over-budget-rejected` | A priced x402 call above the budget is refused, never a signable intent | On the **EIP-3009 funding leg** of `POST /x402/authorize`. Since #2706 the ordinary refusal is the same fail-fast pre-check the erc7710 row below describes → HTTP 403 `delegation_budget_exceeded`; the enforcer's 502 is the path taken when that pre-check **fails open** on a non-chain budget read, and the leg accepts either. Asserting only the 403 would redden specifically when the BACKEND's budget read degrades while this leg's own earlier read did not — most degradations fail it on its own `fromChain` precondition before it ever reaches the fork |
