...
| `pilot:exit-proof` (backend) | #832: the exit acceptance test — enumerate a budget's status, revoke via an owner-signed account op (disableDelegation), confirm redemption then reverts. Uses only public inputs + the owner key; no Haven backend. |
