...
x402 tool-window failures:
   ...
   merchant timeout after funding → MERCHANT_UNRESPONSIVE_AFTER_FUNDING → check_status_later, retry once, then haven_sweep_delegate (eip3009 only; erc7710: nothing to sweep, follow the message)
```
...
| `MERCHANT_UNRESPONSIVE_AFTER_FUNDING` | The merchant did not answer the paid retry before the timeout — NOT proof of rejection, the merchant may still settle late. On eip3009 Haven's funding leg had succeeded; on erc7710 there is no funding leg (#3000). | Both schemes: check `haven_get_payment_status` and retry `haven_complete_mcp_tool` once before doing anything else. eip3009: only then call `haven_sweep_delegate` if no settlement appears. erc7710: nothing to sweep — re-quote only if no settlement appears. |
