...
export const toolDescriptions = {
  ...
  sweep_delegate: {
    summary:
      'Sweep stranded USDC and/or ETH from the delegate wallet back to the originating account.',
    selectionGuidance:
    ...
    behavior:
      ...
      'Each transfer carries confirmation: "confirmed" (a receipt was seen — the funds are in the account) or "unconfirmed" ' +
  ...
  send: {
    ...
    behavior:
      ...
      'The agent\'s signing key signs the account\'s typed data; a current signer refuses any that does not match the payment\'s hash (USEROP_BINDING_MISMATCH), an older signer signs it and the account rejects it on-chain. Haven never receives the key.',
    nextActionGuidance:
