...
export function createEdgeSigner(
  ...
  return {
    ...
    async signSweepAuthorization({
      ...
      //    set without `HAVEN_ACCOUNT_ADDRESS` (the README quickstart — the
      //    credential IS loaded here, it just carries no account address); a
      //    credential file whose `account_address` (or the permanent
      //    `safe_address` fallback) is null; or an embedder calling
